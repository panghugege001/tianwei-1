		...
		<table width="98%" align="center" border="0" cellpadding="4"
			...
			<c:forEach items="${list}" var="s" varStatus="itemIndex">
				<tr bgcolor="#FFFFFF">
					...
					<td height="20" valign="middle" align="center">
						<c:if test="${s.isadmin eq 1}">
							<c:if test="${s.adminstatus eq 0}">(<font color="red">客服管理员</font>)未读</c:if>
							<c:if test="${s.adminstatus eq 1}">(客服管理员)已读</c:if>
						</c:if>
						<c:if test="${s.isadmin eq 0}">
							<c:if test="${s.userstatus eq 0}">(${s.username})未读</c:if>
							<c:if test="${s.userstatus eq 1}">(${s.username})已读</c:if>
						</c:if>
					</td>
					<td height="20" valign="middle" align="center">
						<c:if test="${s.isadmin eq 1}">
					...
					<td height="20" valign="middle" align="center">
						<c:if test="${s.adminstatus eq 0 && s.userstatus eq 1}">
								(客服管理员)没有新的信息
						</c:if>
						<c:if test="${s.adminstatus eq 1 && s.userstatus eq 0}">
								(<font color="red">客服管理员</font>)有新的信息
						</c:if>
						<c:if test="${s.adminstatus eq 1 && s.userstatus eq 1}">
								(客服管理员)没有新的信息
						</c:if>
						<c:if test="${s.adminstatus eq 0 && s.userstatus eq 0}">
								(<font color="red">客服管理员</font>)有新的信息
						</c:if>
					</td>
					<td height="20" valign="middle" align="center">
