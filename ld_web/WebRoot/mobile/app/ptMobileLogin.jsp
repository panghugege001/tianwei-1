...
<body> 
	<script type="text/javascript">
		if(!<%=loginModel%>){
			alert("需登录后才能进入游戏");
			window.location = '<%=request.getRequestURL().toString().replace(request.getServletPath(), "") %>' + "/mobile/app/slotGame.jsp";
		}else{
			var loginname="A${sessionScope.customer.loginname}".toUpperCase();
			...
			window.onload = function(){
				...
				document.getElementById("pth5calloutUrlIn").value = baseUrl+"/mobile/app/slotGame.jsp";
				document.getElementById("pth5Lobby").value = baseUrl+"/mobile/app/slotGame.jsp";
