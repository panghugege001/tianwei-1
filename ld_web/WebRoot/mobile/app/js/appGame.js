...
!(function(w){
    ...
    var GameConfig={
        ...
        loginName:$('#j-LoginName').val().toUpperCase()
    };
    ...
    SlotMg.DataList=[];
    SlotMg.DataUrl={
        PT:['/mobile/json/slot/ptPhone.json?v=100001'],
        PTSW:['/mobile/json/slot/ptsw.json?v=100010'],
        PNG:['/mobile/json/slot/pngPhone.json?v=100001'],
        DT:['/mobile/json/slot/dtPhone.json?v=100001'],
        MGS:['/mobile/json/slot/mgsPhone.json?v=100001'],
        CQ9:['/mobile/json/slot/cq9Phone.json?v=100001'],
        QT:['/mobile/json/slot/qtPhone.json?v=100001'],
        TTG:['/mobile/json/slot/ttgPhone.json?v=100001'],
        NT:['/mobile/json/slot/ntPhone.json?v=100001'],
        AG:['/mobile/json/slot/ag.json?v=100001']
    };
    ...
    SlotMg.ImgUrl={
        ...
        PNG:['/images/pnggames/'],
        DT:['/images/dtgames/'],
        MGS:['/mobile/images/mggames/'],
        CQ9:['/mobile/images/cq9games/'],
        QT:['/mobile/images/qtgames/'],
        TTG:['/images/ttggames/'],
        NT:['/images/ntgames/'],
        AG:['/images/aggames/']
    };
...
//          }
    ...
    SlotMg.reset=function(){
        $filter.find('#slot-game .filter_dropdown_content_sec').each(function(i,e){
            if(i!=0){
                var that = $(this);
        ...
        if(isFun == "0"){
            ...
            switch (obj.category){
                ...
                case 'PTSW':
                    ...
                    break;                    
                case 'MGS':
                    $.post('/mobi/gameH5MGS.aspx',{gameCode:obj.id,fromApp:'app'},function(result){
                        // alert(result)
                        if(result.success){
                ...
                case 'PNG':
                    var gid = obj.id;
                    $.post('/gamePNGFlashForTp.aspx',{practice:0,gameCode:gid,fromApp:'app'},function(result){
                        if(result.success){
                ...
                case 'NT':
                    ...
                    /*if(GameConfig.ntToken){
                        ...
                        $.post('/mobi/getNTGame.aspx',{gameCode:obj.id},function(result){
                            if(result.success){
                                mobileManage.getLoader().close();
...
//                    }
    ...
    SlotMg.load_qtgame=function(gameCode, isfun, gameType, origin){
         if(origin.indexOf('?')){
            origin = origin.substr(0,50);
            console.log(origin)
        }   	
        $.post('/gameQTForTp.aspx',{gameCode:gameCode, isfun:isfun, gameType:gameType, origin:origin, fromApp:'app'},
            function(result){
                if(result.success){
                    window.location.href = result.data.url;
                }else{
    ...
    SlotMg.getLinkDemo=function(obj){
        // pt 没有试玩游戏
        if (obj.category == 'PT' || typeof obj.href != "undefined") {
            return "";
        } else {
...
function setCookie(cname, cvalue) {
    ...
    var d = new Date();
    d.setTime(d.getTime() + (1000*60*10*6)); // 1000毫秒 * 60秒 * 10 * 6 ＝ 60min
...
$(function () {
    ...
    if(platform == "" || typeof platform == 'undefined' || platform == null){
        type = 'PT';
        // type = currentPlatform.toUpperCase();
    } else{
        type = platform;
    }
    ...
    function toggleShowContent(){
        ...
        if ($filterDropdown.hasClass('toggled')) {
            ...
            $filterContent.show().animate({right: '0px'}, 500);
            $filterResetBtns.show().animate({right: '0px'}, 500);
        } else {
            $overlay.removeClass("show");
            $body.removeClass("overflow");

            $filterContent.animate({right: '-1000px'}, 500, function(){
                $filterContent.hide();
            });
            $filterResetBtns.hide().animate({right: '-1000px'}, 800);
        }
        return false;
    }
    ...
    $("header").hide();
    // $("#comm-back-button").css("top","12px").hide();
    // $("#comm-header-title").html("游戏大厅");
});
