	        ...
	        '<div class="m-grid-footer">',
				'<div class="m-grid-pagination">',
			        '<div class="m-grid-pagination-left">',
						'<div class="m-grid-pagination-first iconfont icon-arrow-previous-2"></div>',
						'<div class="m-grid-pagination-prev iconfont icon-arrow-previous"></div>',
					'</div>',
					...
					'<div class="m-grid-pagination-right">',
						'<div class="m-grid-pagination-next iconfont icon-arrow-next"></div>',
						'<div class="m-grid-pagination-last iconfont icon-arrow-next-2"></div>',
					'</div>',
				'</div>',
	...
	function _load(){
        ...
        mobileManage.ajax({
            ...
            dataFilter:function(res){
                var $res=eval('('+res+')')
			    var data={};
				if($res instanceof Array||$res.toString()==='[object Array]'){
                    data=JSON.stringify({success:true,data:{records:$res,pageIndex:1,size:$res.length, total:$res.length},message:""})
                }else{
                    data=res;
                }
				return data;
