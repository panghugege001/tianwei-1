...
function TPPManage(base_url) {
    ...
    var _payUrl =  _base_url+'mobi/thirdPartyPayment.aspx';
    var _payUrl2 =  _base_url+'asp/pay_api.aspx';
    var _payＷay =  _base_url+'asp/pay_way.aspx';
    var _payDatasUrl = _base_url+'mobi/getWorkThirdPartyPayments.aspx';
    var _payWeixinUrl = _base_url+'mobi/getWorkWeixin.aspx';
    var _payDCardUrl = _base_url+'mobi/dCardPay.aspx';
    var _getDCardPayWorkUrl = _base_url+'mobi/getDCardPayWork.aspx';
    var _getAllPaymentsUrl = _base_url+'mobi/getAllPayments.aspx';
    var _payZfbqr2Url = _base_url+'mobi/getWorkZfbqr2.aspx';
...
//	var _formHtml = '<form action="{0}" method="post" target="_blank">{1}</form>';
    ...
    that.payCardTo = function (formData) {
        var err = _payCardCheck2(formData);
        if(err){
            alert(err);
            return;
        }
    ...
    that.pay = function(formData,callback){
        ...
        mobileManage.ajax({
            ...
            callback:function (result) {
                if(result.success){
                    if("http://www.xinyisifuzhuangmy.com/ybpay.jsp"==result.data.url){  //银宝支付宝
                        result.data.url = CreateYBZFBURL(result.data.url,result.data.formData);
                    }
                    if("http://apika.10001000.com/chargebank.aspx"==result.data.url){  //千网支付宝
    ...
    //点卡支付資料驗證
    function _payCardCheck2(formData){
