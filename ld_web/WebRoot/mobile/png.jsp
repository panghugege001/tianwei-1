...
<html>
	...
	<body> 
		 <div class="main-wrap">
			...
			<div class="content">
				<div class="tab-block">
					<div data-tag="" class="tab-button mui-col-xs32-6 active">PNG游戏</div>
...
// 			});
			...
			function queryGames(){
				var gameTemplateHtml = [
					...
					'<img src="/images/pnggames/{{pic}}">',
					'<div class="name">{{name}}</div>',
				    '<div class="buttons">',
						  '<a href="javascript:play_game(\'{{id}}\')" >立即游戏</a>',
			    ...
			    mobileManage.ajax({
			    	url:'mobile/json/game/pngPhone.json?v=0101',
...
//			正式遊戲
			function play_game(id){
                if('${session.customer==null}'=='true'){
                    ...
                    mobileManage.ajax({
                        url:'mobi/gamePNGMobile.aspx',
                        param:{practice:0,gameCode:id},
                        callback:function(result){
                            if(result.success){
...
//          試玩遊戲			
			function play_guest_game(id,code){
                var gameUri = "https://bsicw.playngonetwork.com/casino/PlayMobile?pid={{code}}&gid={{id}}&lang=zh_CN&practice=1";
