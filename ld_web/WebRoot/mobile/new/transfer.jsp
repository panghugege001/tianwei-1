...
<link rel="stylesheet" type="text/css"
	href="/mobile/css/new/account.css" />
</head>
...
<body style="background: #eee;">
	...
	<div id="tab-warp">
		<div id="funds-transfer-page">
			<div class="form-warp no-icon">
				<div class="form-tips">
					来源账户 <small>(余额 <span id='transfer-source-credit'
						class="c-red">0.00</span> <img style="width: 1rem;" class="hidden"
						src="/mobile/img/loading.gif" alt="" />元)
					</small>
				</div>
				...
				<div class="form-tips">
					目标账户<small>(余额 <span id="transfer-target-credit"
						class="c-red">0.00</span> <img style="width: 1rem;" class="hidden"
						src="/mobile/img/loading.gif" alt="" /> 元)
					</small>
				</div>
				...
				<div class="form-group ">
					<input type="text" id="transfer-money" class="form-control"
						placeholder="0.00" />
				</div>
				...
				<div class="text-tips">
					...
					<ol>
						<li>老虎机账户"支持BBIN、PG、CQ9、MG、DT，若要游玩以上平台请将额度转入对应游戏平台账户即可。</li>
		...
		<div class="form-warp no-icon hidden">
			...
			<div class="form-group">
				<select id="redRainOut" onchange="redRainMoneryOut(this.value);"
					class="form-control">
			...
			<div class="form-group">
				...
				<input id="redRainOutDiv" type="text" class="form-control" disabled
					readonly value="0.00" />
			...
			<div class="form-group  ">
				<select id="redRainIn" onchange="redRainMoneryIn(this.value);"
					class="form-control">
				</select>
			</div>
			<label class="form-tips">目标余额：</label>
			<div class="form-group ">
				<input id="redRainInDiv" type="text" class="form-control" disabled
					readonly value="0.00" />
			</div>
			<label class="form-tips">转账金额：</label>
			<div class="form-group ">
				<input id="redRainMoney" class="form-control" type="text"
					placeholder="请输入需要转账的金额" />
			</div>
	...
	<script type="text/javascript">
		...
		function TransferPage() {
			...
			var $transferSource = $("#transfer-source"), $transferTarget = $("#transfer-target");
			//来原账户下拉
			$.each(that.accountData, function(i, item) {
				$transferSource.append('<option value="' + item.value + '">'
						+ item.name + '</option>')
			})
			$transferSource
					.change(function() {
			...
			$.each(that.xialaData, function(i, item) {
				$transferTarget.append('<option value="' + item.value + '">'
						+ item.name + '</option>')
			})
			...
			function doTransger() {
				...
				mobileManage.getBankManage().transfer(formData,
						function(result) {
							if (result.success) {
			...
			function queryCredit() {
				//先查询天威平台余额
				mobileManage.getUserManage().getCredit(function(result) {
					if (result.success) {
