    ...
    $.get('/points/getAllPointsPresents.aspx', {}, function (reqData) {
        if (reqData.code === CODE) {
            ...
            $.getJSON('/points/getLevelSave.aspx', function (data) {
                var html = ''
                var $box = $('.integral-promotion ul');
                if (data.code === CODE) {
                    for (var i = 0; i < data.data.length; i++) {
                        var item = data.data[i];
                        var value = item.value / 10 === 10 ? '无折扣' : item.value / 10 + '折'
                        html += '<li class="promotion-list"><span>' + item.note + '</span><span>' + value + '</span></li>'
                    }
            ...
            $('.exchange').click(function () {
                var point = $('.properties span.active').attr('data-point')
                if (!isLogin()) return
                if (point > myTotalPonits) {//积分不够
                    ...
                    layer.open({
                        ...
                        content: html,
                        yes: function () {
                            window.open('/asp/payPage.aspx?#tab_deposit')
                        }
                ...
                if (type === 'physical') {//实物
                    window.open('./address.jsp?id=' + id)
                } else if (type === 'phoneData' || type === 'phoneFee') {//手机
                    var html = '<div>\
                                <p>请输入您将要充值的的手机号</p>\
                                手机号：<input class="phone" type="text" value=""/>\
