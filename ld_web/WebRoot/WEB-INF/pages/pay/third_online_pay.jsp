		...
		<form id="dinpayRedirect" action="${ctx}/asp/pay_api.aspx" method=post name="dinpayRedirect" id="dinpayRedirect">
			...
			<table class="account-info table-pay">
				...
				<tr>
					...
					<td>
						<div class="ipt-group">
							<select name="bankCode" id="bankco" class="ipt-txt"></select>
							<span class="c-red">* 必填</span>
						</div>
					</td>
				</tr>
        ...
        $(function () {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (obj.code == "10000") {
                        $("#bankco").empty();
                        $("#bankco").append("<option value=''>－ 请选择银行 －</option>");
                        $.each(obj.data, function (index, vo) {
                            $("#bankco").append("<option value=" + vo.dictValue + ">" + vo.dictName + "</option>");
                        });
                    }
                }
            });
