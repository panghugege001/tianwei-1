...
<%@page import="dfh.model.PayConst"%>
<%
	/* *
	 功能：智付页面跳转同步通知页面
	 版本：3.0
	...
	String key = "";
	if(merchant_code.equals(PayConst.getMerchantNo1())){
	      key = PayConst.getMerchantKey1();
	}else if(merchant_code.equals(PayConst.getMerchantNo2())){
	      key = PayConst.getMerchantKey2();
	}else if(merchant_code.equals(PayConst.getMerchantNo3())){
		  key = PayConst.getMerchantKey3();
	}else if(merchant_code.equals(PayConst.getMerchantNo4())){
		  key = PayConst.getMerchantKey4();
	}
	...
	if (dinpaySign.equals(sign)) {
		...
		if (trade_status.equals("SUCCESS")) {
			...
			String returnmsg = AxisSecurityEncryptUtil.addPayorderZf(trade_no, Double.parseDouble(order_amount), extra_return_param, bank_seq_no, trade_time,merchant_code);
			if (returnmsg == null) {
				out.println("交易成功,你支付的" + order_amount + "元已经到账,此次交易的订单号为" + trade_no);
			} else {
