...
import dfh.action.tpp.paramenum.KDWXFormParamEnum;
...
public class KDWXThirdPartyPayment extends ThirdPartyPayment{

	DecimalFormat df = new DecimalFormat("#0");
	KDWXThirdPartyPayment(String id,String name,String url){
		this.id = id;
		this.name = name;
		this.url = url;
	}
	...
	public boolean verification(Users user, String bankId, double money) {
		try {
			...
			//获取商家订单号
			String orderNo = AxisUtil.getObjectOne(AxisUtil.getClient(AxisUtil.PUBLICWEBSERVICEURL + "UserWebService", false), AxisUtil.NAMESPACE, "getKdWxZfsOrderNo", new Object[] {user.getLoginname(),money,"kdwxzf"}, String.class);
			if (orderNo == null) {
				message = "[提示]获取商家订单号失败！";
				return false;
			}

			formData = KDWXFormParamEnum.toMap();

			// 定单金额（必填）
			formData.put(KDWXFormParamEnum.P_Price, df.format(money));
			formData.put(KDWXFormParamEnum.P_FaceValue, df.format(money));

			formData.put(KDWXFormParamEnum.P_OrderId,orderNo);
			formData.put(KDWXFormParamEnum.P_Notic,"wap_"+user.getLoginname());
			
			Set<String> noAppendValue = new HashSet<String>();
			noAppendValue.add(KDWXFormParamEnum.P_FaceValue.toString());
			noAppendValue.add(KDWXFormParamEnum.P_Quantity.toString());
			noAppendValue.add(KDWXFormParamEnum.P_Description.toString());
			noAppendValue.add(KDWXFormParamEnum.P_Notic.toString());
			noAppendValue.add(KDWXFormParamEnum.P_Result_URL.toString());
			noAppendValue.add(KDWXFormParamEnum.P_Notify_URL.toString());
			noAppendValue.add(KDWXFormParamEnum.P_PostKey.toString());
			
			StringBuffer signSrc = new StringBuffer(); 
			for(Enum p : KDWXFormParamEnum.values()){
				Object value = formData.get(p);
				if(!noAppendValue.contains(p.toString())){
					signSrc.append(value+"|");
				}
			}
			signSrc.append("2fed5e622429470fbbe7879ec8f6f315");
			String singInfo = EncryptionUtil.encryptPassword(signSrc.toString());
			formData.put(KDWXFormParamEnum.P_PostKey,singInfo);
			return true;
		} catch (Exception e) {
			e.printStackTrace();
			message = "网络繁忙,请稍后再试！";
