...
public class HfpayAction1 extends SubActionSupport {
	...
	public String hfRedirect() {
		try {
			...
			payType=(String)request.getSession().getAttribute("hfConstId");
			payType=payType.replace("'", "");
			request.getSession().removeAttribute("hfConstId");//把支付方式从session中移除
			// 判断在线支付是否存在
			Const constPay = AxisUtil.getObjectOne(AxisUtil.getClient(AxisUtil.PUBLICWEBSERVICEURL + "UserWebService", false), AxisUtil.NAMESPACE, "selectDeposit", new Object[] {payType }, Const.class);
			if (constPay == null) {
			...
			// 获取商家订单号
			String orderNo = AxisUtil.getObjectOne(AxisUtil.getClient(AxisUtil.PUBLICWEBSERVICEURL + "UserWebService", false), AxisUtil.NAMESPACE, "getOrderHfNo1", new Object[] {user.getLoginname(),Double.parseDouble(OrdAmt),constPay.getId()}, String.class);
			if (orderNo == null) {
				setError_info("[提示]获取商家订单号失败！");
				return ERROR;
			}
			// 查询商号
			PayMerchant payMer = AxisUtil.getObjectOne(AxisUtil.getClient(AxisUtil.PUBLICWEBSERVICEURL + "UserWebService", false), AxisUtil.NAMESPACE, "selectPayMerchantById", new Object[] {payType}, PayMerchant.class);
			if(null==payMer||null==payMer.getId()){
				setError_info("[提示]获取商号信息失败！");
				return ERROR;
			}
			/** 必须* */
			Version = "10";
			...
			PnrNum = "";
			/** 必须* */
			BgRetUrl = "http://pay.jiekoue68.com:2112/asp/payHfReturn1.aspx";
			...
			String keyString="MerPrK"+payMer.getMerchantcode()+".key";//该商户号对应的Key 秘钥
			String path = this.getClass().getResource("").toURI().getPath() + keyString;
