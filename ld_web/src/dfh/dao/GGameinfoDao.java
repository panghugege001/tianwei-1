...
public class GGameinfoDao extends HibernateDaoSupport  {
	...
	public List searchXimaDetail(String loginname, Date startTime, Date endTime,int offset,int length)throws Exception{
		String hql="select a.pno,a.type,a.flag,b.firstcash,b.trycredit,b.rate,b.starttime,b.endtime from proposal a,xima b where a.pno=b.pno and b.endtime>=? and b.endtime<? and a.loginname=? order by endtime desc";
	...
	public AutoXima getTotalCount(String loginname,Date startTime,Date endTime)throws Exception{
		String hql="select count(*) totalCount,sum(b.firstcash) firstcash,sum(b.trycredit) trycredit from proposal a,xima b where a.pno=b.pno and b.endtime>=? and b.endtime<? and a.loginname=?";
		SQLQuery query = this.getSession().createSQLQuery(hql);
		 query.setParameter(0, startTime);
		 query.setParameter(1, endTime);
		 query.setParameter(2, loginname);
