...
public class QTDepositServiceImpl extends BaseService implements IPlatformDepositService {
	...
	public PlatformDepositVO firstDeposit(PlatformDepositVO vo) {
		...
		log.info("QTDepositServiceImpl类的firstDeposit方法参数为：【id=" + id + ",platform=" + platform + ",youhuiType=" + youhuiType + ",loginName=" + loginName + ",remit=" + remit + "】");
		...
		if (!user.getWarnflag().equals(3)) {
			...
			if (flag) {
				...
				vo.setMessage("抱歉，您已申请过自助QT首存优惠，不能重复申请此优惠！");
				return vo;
			}
		}
		...
		String remark = proposalRemark + "自助QT首存优惠," + config.getBetMultiples() + "倍流水,存" + remit + "送" + changeMoney;
		
		String transferId = sequenceService.generateTransferId();
		
		String msg = transferInfoService.transferQT(transferId, loginName, remit, remark);
		...
		Double betAmount = queryBetAmountFromPLATFORMDATA(baseDao.getHibernateTemplate(), loginName, "qt");

		PreferentialRecord record = new PreferentialRecord(pno, loginName, "qt", betAmount, new Date(), 0);
		baseDao.save(record);
		...
		selfRecord.setPlatform("qt");
		selfRecord.setSelfname("自助QT首存优惠");
		...
		vo.setTransferId(transferId);
		vo.setGiftMoney(Arith.add(changeMoney, remit));
		vo.setMessage("自助QT首存优惠成功！");
	...
	public PlatformDepositVO timesDeposit(PlatformDepositVO vo) {
		...
		log.info("QTDepositServiceImpl类的timesDeposit方法参数为：【id=" + id + ",platform=" + platform + ",youhuiType=" + youhuiType + ",loginName=" + loginName + ",remit=" + remit + "】");
		...
		String remark = proposalRemark + "自助QT次存优惠," + config.getBetMultiples() + "倍流水,存" + remit + "送" + changeMoney;
		...
		selfRecord.setLoginname(loginName);
		selfRecord.setPlatform("qt");
		selfRecord.setSelfname("自助QT次存优惠");
		selfRecord.setType(0);
		selfRecord.setCreatetime(new Date());
	...
	public PlatformDepositVO limitedTime(PlatformDepositVO vo) {
		...
		log.info("QTDepositServiceImpl类的limitedTime方法参数为：【id=" + id + ",platform=" + platform + ",youhuiType=" + youhuiType + ",loginName=" + loginName + ",remit=" + remit + "】");
		...
		String remark = proposalRemark + "自助QT限时优惠," + config.getBetMultiples() + "倍流水,存" + remit + "送" + changeMoney;
		...
		selfRecord.setLoginname(loginName);
		selfRecord.setPlatform("qt");
		selfRecord.setSelfname("自助QT限时优惠");
		selfRecord.setType(0);
		selfRecord.setCreatetime(new Date());
