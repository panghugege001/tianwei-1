...
public class TTGDepositServiceImpl extends BaseService implements IPlatformDepositService {

	private static Logger log = Logger.getLogger(TTGDepositServiceImpl.class);
	...
	public PlatformDepositVO firstDeposit(PlatformDepositVO vo) {
		...
		log.info("TTGDepositServiceImpl类的firstDeposit方法参数为：【id=" + id + ",platform=" + platform + ",youhuiType=" + youhuiType + ",loginName=" + loginName + ",remit=" + remit + "】");
		...
		if (!user.getWarnflag().equals(3)) {
			...
			if (flag) {
				...
				vo.setMessage("抱歉，您已申请过自助TTG首存优惠，不能重复申请此优惠！");
				return vo;
			}
		}
		...
		if (StringUtils.isBlank(money) || !NumberUtils.isNumber(money)) {
			...
			vo.setMessage("获取玩家TTG平台余额失败，请稍后重试！");
			return vo;
		}

		// 后期远程金额
		Double remoteCredit = Double.valueOf(money);
		...
		String remark = proposalRemark + "自助TTG首存优惠," + config.getBetMultiples() + "倍流水,存" + remit + "送" + changeMoney;
		
		String transferId = sequenceService.generateTransferId();
		
		String msg = transferInfoService.transferTTG(transferId, loginName, remit, remark);
		...
		Double betAmount = queryBetAmountFromPLATFORMDATA(baseDao.getHibernateTemplate(), loginName, "ttg");

		PreferentialRecord record = new PreferentialRecord(pno, loginName, "ttg", betAmount, new Date(), 0);
		baseDao.save(record);
		...
		selfRecord.setPlatform("ttg");
		selfRecord.setSelfname("自助TTG首存优惠");
	...
	public PlatformDepositVO timesDeposit(PlatformDepositVO vo) {
		...
		log.info("TTGDepositServiceImpl类的timesDeposit方法参数为：【id=" + id + ",platform=" + platform + ",youhuiType=" + youhuiType + ",loginName=" + loginName + ",remit=" + remit + "】");
		...
		String remark = proposalRemark + "自助TTG次存优惠," + config.getBetMultiples() + "倍流水,存" + remit + "送" + changeMoney;
		...
		selfRecord.setLoginname(loginName);
		selfRecord.setPlatform("ttg");
		selfRecord.setSelfname("自助TTG次存优惠");
		selfRecord.setType(0);
		selfRecord.setCreatetime(new Date());
	...
	public PlatformDepositVO limitedTime(PlatformDepositVO vo) {
		...
		String remark = proposalRemark + "自助TTG限时优惠," + config.getBetMultiples() + "倍流水,存" + remit + "送" + changeMoney;
		...
		selfRecord.setLoginname(loginName);
		selfRecord.setPlatform("ttg");
		selfRecord.setSelfname("自助TTG限时优惠");
		selfRecord.setType(0);
		selfRecord.setCreatetime(new Date());
