...
public class MGDepositServiceImpl extends BaseService implements IPlatformDepositService {

	private static Logger log = Logger.getLogger(MGDepositServiceImpl.class);
	...
	public PlatformDepositVO firstDeposit(PlatformDepositVO vo) {
		...
		log.info("MGDepositServiceImpl类的firstDeposit方法参数为：【id=" + id + ",platform=" + platform + ",youhuiType=" + youhuiType + ",loginName=" + loginName + ",remit=" + remit + "】");
		...
		if (!user.getWarnflag().equals(3)) {
			...
			if (flag) {
				...
				vo.setMessage("抱歉，您已申请过自助MG首存优惠，不能重复申请此优惠！");
				return vo;
			}
		}
		...
		try {
			...
			if (null == remoteCredit) {
				...
				vo.setMessage("获取玩家MG平台余额失败，请稍后重试！");
				return vo;
			}
		} catch (Exception e) {
			
			vo.setMessage("获取玩家MG平台余额发生异常，请稍后重试！");
			return vo;
		}
		
		if (remoteCredit >= 5) {
			
			vo.setMessage("MG平台余额必须小于5元，才能自助优惠！");
			return vo;
		}
		...
		String remark = proposalRemark + "自助MG首存优惠," + config.getBetMultiples() + "倍流水，存" + remit + "送" + changeMoney;
		
		String transferId = sequenceService.generateTransferId();
		
		String msg = transferInfoService.transferMG(transferId, loginName, remit, remark);
		...
		PreferentialRecord record = new PreferentialRecord(pno, loginName, "mg", null, new Date(), 0);
		baseDao.save(record);
		...
		selfRecord.setPlatform("mg");
		selfRecord.setSelfname("自助MG首存优惠");
	...
	public PlatformDepositVO timesDeposit(PlatformDepositVO vo) {
		...
		log.info("MGDepositServiceImpl类的timesDeposit方法参数为：【id=" + id + ",platform=" + platform + ",youhuiType=" + youhuiType + ",loginName=" + loginName + ",remit=" + remit + "】");
		...
		String remark = proposalRemark + "自助MG次存优惠," + config.getBetMultiples() + "倍流水,存" + remit + "送" + changeMoney;
		...
		selfRecord.setLoginname(loginName);
		selfRecord.setPlatform("mg");
		selfRecord.setSelfname("自助MG次存优惠");
		selfRecord.setType(0);
		selfRecord.setCreatetime(new Date());
	...
	public PlatformDepositVO limitedTime(PlatformDepositVO vo) {
		...
		log.info("MGDepositServiceImpl类的limitedTime方法参数为：【id=" + id + ",platform=" + platform + ",youhuiType=" + youhuiType + ",loginName=" + loginName + ",remit=" + remit + "】");
		...
		String remark = proposalRemark + "自助MG限时优惠," + config.getBetMultiples() + "倍流水，存" + remit + "送" + changeMoney;
		...
		selfRecord.setLoginname(loginName);
		selfRecord.setPlatform("mg");
		selfRecord.setSelfname("自助MG限时优惠");
		selfRecord.setType(0);
		selfRecord.setCreatetime(new Date());
