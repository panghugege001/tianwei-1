...
import dfh.exception.LiveOrderExecuteException;
import dfh.exception.LiveTimeOutException;
import dfh.response.SbRespBody;
...
public class SbSportUtil {
	...
	// 手机登录
	private static final String MOBILE_LOGIN_URL = "http://smartsbtest.l0030.ig128.com";
	// PC登录
	private static final String PC_LOGIN_URL = "http://sbtest.l0030.ig128.com";
	...
	public String free(String gameType, String ip, Boolean isMobile, String baseUrl) throws Exception {
		throw new Exception("[SB]不可以试玩！");
	}

	public static String play(String account, String gameType, String ip, Boolean isMobile) throws Exception {
		checkAndCreateMember(account);
		String token = getToken(account);
		String url = PC_LOGIN_URL;
		// 1 mobile 默认PC
		if (isMobile) {
			url = MOBILE_LOGIN_URL;
		}
		return url + "/deposit_processLogin.aspx?webskintype=2&lang=cs" + "&token=" + token;
	}
	 ...
	 * @throws Exception
	 */
	public static Map getBetRecord(String versionKey) throws Exception {
		Map<String, String> reqMap = new HashMap<>();
		reqMap.put("vendor_id", VENDOR_ID);
		reqMap.put("version_key", versionKey);
		reqMap.put("options", "");
		try {
			logger.info("[SB]注单拉取，请求参数：" + reqMap);
			return (Map) doPost("/GetBetDetail", reqMap);
		} catch (LiveException ex) {
			logger.error("[SB]注单拉取，异常：", ex);
			throw ex;
		} catch (Exception ex) {
			logger.error("[SB]注单拉取，异常：" + reqMap, ex);
			throw ex;
		}
	}
	...
	public static void main(String[] args) {
		try {
			System.out.print(getBetRecord("0"));
		} catch (Exception e) {
