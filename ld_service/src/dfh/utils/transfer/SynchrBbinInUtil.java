...
public class SynchrBbinInUtil {
	
	private static Logger log = Logger.getLogger(SynchrBbinInUtil.class);
	
	private static SynchrBbinInUtil instance = null;
	
	private SynchrBbinInUtil(){
		
	}
	
	public static SynchrBbinInUtil getInstance() {
		if (instance == null) {
			return instance = new SynchrBbinInUtil();
		} else {
			return instance;
		}
	}
	
	public synchronized String transferInforBbin(TransferService transferService , String seqId, String loginname, Double remit){
		//转账前先判断额度是否可以获取到
		try {
			Object output = (loginname != null ? RemoteCaller.queryBbinCredit(loginname) : null);
		...
		String msg = transferService.transferInforBbinIn(seqId, loginname, remit);
		if (msg == null) {
			...
			if (oldCredit!=newCredit) {
				try {
					log.info(loginname+" BBIN转账额度不同：******"+loginname+"******旧额度:"+oldCredit+"******新额度:"+newCredit+"******改变额度:"+remit);
					DspResponseBean dspResponseBean = RemoteCaller.depositBbinRequest(loginname, remit.intValue(), oldCredit, seqId);
					if(dspResponseBean==null){
						log.info(loginname+"转账发生异常,请稍后再试或联系在线客服");
						return "转账发生异常,请稍后再试或联系在线客服";
					}else if (dspResponseBean != null && dspResponseBean.getInfo().equals("11100")) {
						transferService.addTransferforBbin(Long.parseLong(seqId), loginname, oldCredit, remit, Constants.IN, Constants.FAIL, "", "转入成功");
						log.info(loginname+" BBIN转账成功！");
						return null;
					} else if(dspResponseBean != null){
						// 转账失败 退还额度
						/*Creditlogs creditLog = new Creditlogs();
						creditLog.setLoginname(loginname);
						creditLog.setType(CreditChangeType.TRANSFER_BBININ.getCode());
						creditLog.setCredit(newCredit);
						creditLog.setRemit(remit);
						creditLog.setNewCredit(newCredit + remit);
						creditLog.setRemark("BBIN转账错误!自动退还:" + seqId + "订单");
						...
						String info="BBIN转账错误!改变前订单:" + seqId +" 改变前:" + oldCredit + "改变后:" + (oldCredit - remit) + " 改变额度:" + remit+" 自动退还订单:" + seqId + " 改变前:" + newCredit + "改变后:" + (newCredit + remit) + " 改变额度:" + remit+" 系统已经处理 请检查是有否错误";
...
//						transferService.updateUserCreditSql(user1, remit);
						...
						log.info("转账状态错误:" + dspResponseBean.getInfo()+"单号:" + seqId + "***账户:" + loginname);
				...
				log.info("BBIN转账额度相同 转账额度出现问题:******"+loginname+"******旧额度:"+oldCredit+"******新额度:"+user1.getCredit());
