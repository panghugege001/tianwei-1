 ...
 *
 */
public class SynchrTtInUtil {

	private static Logger log = Logger.getLogger(SynchrTtInUtil.class);

	private static SynchrTtInUtil instance = null;

	private SynchrTtInUtil() {

	}

	public static SynchrTtInUtil getInstance() {
		if (instance == null) {
			return instance = new SynchrTtInUtil();
		} else {
			return instance;
		}
	}
				...
				if(null != transfers && transfers.size()>0){
					...
					if(null != transfer){
						if(transfer.getRemark().contains("自助优惠")){  //最后一笔是体验金的转账记录
							return "您正在使用体验金，TTG余额大于等于100或者小于1的时候才能进行户内转账";
						}
					}
				}
