...
public class SynchrKenoInUtil {
	
	private static SynchrKenoInUtil instance = null;
	
	private static Logger log = Logger.getLogger(SynchrKenoInUtil.class);
	
	private SynchrKenoInUtil(){
		
	}
	
	public static SynchrKenoInUtil getInstance() {
		if (instance == null) {
			return instance = new SynchrKenoInUtil();
		} else {
			return instance;
		}
	}
	
	public synchronized String transferInforkeno(TransferService transferService , String seqId, String loginname, Double remit ,String ip){
		// 转账前先判断额度是否可以获取到
		try {
		...
		String msg = transferService.transferInforkenoIn(seqId, loginname, remit, ip);
		if (msg == null) {
			...
			if (oldCredit != newCredit) {
				try {
					log.info(loginname + " Keno转账额度不同：******" + loginname + "******旧额度:" + oldCredit + "******新额度:" + newCredit + "******改变额度:" + remit);
					...
					if (bean != null) {
						if (bean.getName() != null && bean.getName().equals("Error")) {
							...
							creditLog.setType(CreditChangeType.TRANSFER_KENOIN.getCode());
							creditLog.setCredit(newCredit);
							creditLog.setRemit(remit);
							creditLog.setNewCredit(newCredit + remit);
							creditLog.setRemark("KENO转账错误!自动退还:" + seqId + "订单");
							...
							*/
							...
							String info = "KENO转账错误!改变前订单:" + seqId + " 改变前:" + oldCredit + "改变后:" + (oldCredit - remit) + " 改变额度:" + remit + " 自动退还订单:" + seqId + " 改变前:" + newCredit + "改变后:" + (newCredit + remit) + " 改变额度:" + remit + " 系统已经处理 请检查是有否错误";
							...
							if (FundIntegrationId != null) {
								KenoResponseBean confirmbean = DocumentParser.parseKenologinResponseRequest(KenoUtil.transferconfirm(loginname, remit, ip, FundIntegrationId, seqId));
								if(confirmbean==null){
								...
								if (confirmbean.getName() != null && confirmbean.getName().equals("Remain")) {
									transferService.addTransferforKneo(Long.parseLong(seqId), loginname, oldCredit, remit, Constants.IN, Constants.SUCESS, "", "转入成功");
				...
				log.info("KENO转账额度相同 转账额度出现问题:******" + loginname + "******旧额度:" + oldCredit + "******新额度:" + user1.getCredit());
