...
public class SynchrSportInUtil {

	private static Logger log = Logger.getLogger(SynchrSportInUtil.class);

	private static SynchrSportInUtil instance = null;

	private SynchrSportInUtil() {

	}

	public static SynchrSportInUtil getInstance() {
		if (instance == null) {
			return instance = new SynchrSportInUtil();
		} else {
			return instance;
		}
	}

	public synchronized String transferInforTy(TransferService transferService, String seqId, String loginname, Double remit) {
		// 转账前先判断额度是否可以获取到
		try {
