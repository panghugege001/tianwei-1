...
public class SelfYouHuiServiceImpl implements ISelfYouHuiService {
	...
	//全民闯关金额转入pt
		public String selfTransferPtEmigrated(String loginname , Double remit ){
			...
			if(null != offers && offers.size()>0){
				...
				Proposal proposal = new Proposal(pno, "system", DateUtil.now(), ProposalType.EMIGRATED391.getCode(), user.getLevel(), loginname , remit, user.getAgent(), ProposalFlagType.EXCUTED.getCode(), Constants.FROM_FRONT, null, null);
	...
	public String selfTransferEaActivity(String loginname, Double remit,ActivityConfig config) {
		...
		logDao.insertCreditLog(user.getLoginname(), ProposalType.ACTIVITY442.getText(), user.getCredit(), remit, user.getCredit(), "referenceNo:" + pno + ";" + StringUtils.trimToEmpty(remark));
		...
		try {
			DepositPendingResponseBean depositPendingResponseBean = RemoteCaller.depositPendingRequest(loginname, remit, transID, user.getAgcode());
			if(null != depositPendingResponseBean && depositPendingResponseBean.getStatus().equals(ErrorCode.SUCCESS.getCode())){
				RemoteCaller.depositConfirmationResponse(depositPendingResponseBean.getId(), ErrorCode.SUCCESS.getCode(), depositPendingResponseBean.getPaymentid(), user.getAgcode());

				transferService.addTransfer(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, ProposalType.ACTIVITY442.getText(), remark);
		...
		try {
			...
			if (dspResponseBean != null && dspResponseBean.getInfo().equals("0")) {
				...
				if(null != dspConfirmResponseBean && dspConfirmResponseBean.getInfo().equals("0")){
					transferService.addTransferforAginDsp(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, ProposalType.ACTIVITY442.getText(), remark);
					return null;
				}else{
					return "转入失败";
				}
	...
	public String selfTransferMgActivity(String loginname, Double remit, ActivityConfig config) {
		...
		try {
			transferService.addTransferforMg(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, ProposalType.ACTIVITY442.getText(), remark);
			return null;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String selfTransferDtActivity(String loginname, Double remit,ActivityConfig config) {
		...
		try {
			transferService.addTransferforDt(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, ProposalType.ACTIVITY442.getText(), remark);
			return null;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String selfTransferPngActivity(String loginname, Double remit,ActivityConfig config) {
		...
		try {
			transferService.addTransferforPng(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, "", remark);
			return null;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String selfTransferTtgActivity(String loginname, Double remit,ActivityConfig config) {
		...
		try {
			...
			//if(null != b && b){
			transferService.addTransferforTt(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, ProposalType.ACTIVITY442.getText(), remark);
			return null;
			/*}else{
				vo.setMessage("奖金转入ttg失败  ， 数据回滚。");
				throw new GenericDfhRuntimeException("奖金转入ttg失败  ， 数据回滚。");
			}*/
		} catch (Exception e) {
			e.printStackTrace();
			return "系统繁忙";
	...
	public String selfTransferPtActivity(String loginname , Double remit,ActivityConfig config ){
		...
		String transID = seqService.generateTransferID();
		Offer offer = new Offer(pno, UserRole.MONEY_CUSTOMER.getCode(), loginname, 0.0, remit , remark);
		Proposal proposal = new Proposal(pno, "system", DateUtil.now(), ProposalType.ACTIVITY442.getCode(), user.getLevel(), loginname , remit, user.getAgent(), ProposalFlagType.EXCUTED.getCode(), Constants.FROM_FRONT, remark, null);
		...
		/*	if (deposit != null && deposit) {
				...
				vo.setMessage("签到奖金转入PT失败，请联系客服！");
				throw new GenericDfhRuntimeException(
						"签到奖金转入PT失败  ， 数据回滚。");
			}*/
		transferService.addTransferforNewPt(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL,ProposalType.ACTIVITY442.getText(), remark);
	...
	public String selfTransferNTActivity(String loginname, Double remit,ActivityConfig config) {
		...
		try {
			transferService.addTransferforNT(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, ProposalType.ACTIVITY442.getText(), remark);
			return null;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String selfTransferQTActivity(String loginname, Double remit,ActivityConfig config) {
		...
		try {
			transferService.addTransferforQt(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, ProposalType.ACTIVITY442.getText(), remark);
			return null;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	/**
		...
		public String selfTransferNTEmigrated(String loginname, Double remit) {
			...
			try {
				transferService.addTransferforNT(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, "", remark);
				return null;
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public String selfTransferTtgHB(String loginname, Double remit,String betMultiples,Integer deposit) {
		...
		Offer offer = new Offer(pno, UserRole.MONEY_CUSTOMER.getCode(), loginname, 0.0, remit, remark);
		Proposal proposal = new Proposal(pno, "system", DateUtil.now(), ProposalType.HB499.getCode(),
				user.getLevel(), loginname, remit, user.getAgent(), ProposalFlagType.EXCUTED.getCode(),
				Constants.FROM_FRONT, null, null);
		...
		try {
			transferService.addTransferforTt(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, "从红包账户转至TTG账户", remark);
			return null;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String selfTransferPtHB(String loginname , Double remit,String betMultiples,Integer deposit ){
		...
		Offer offer = new Offer(pno, UserRole.MONEY_CUSTOMER.getCode(), loginname, 0.0, remit , remark);
		Proposal proposal = new Proposal(pno, "system", DateUtil.now(), ProposalType.HB499.getCode(), user.getLevel(), loginname , remit, user.getAgent(), ProposalFlagType.EXCUTED.getCode(), Constants.FROM_FRONT, null, null);
	...
	public String selfTransferNTHB(String loginname, Double remit,String betMultiples,Integer deposit) {
		...
		try {
			transferService.addTransferforNT(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, "从红包账户转至NT账户", remark);
			return null;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String selfTransferQtHB(String loginname, Double remit,String betMultiples,Integer deposit) {
		...
		try {
			transferService.addTransferforQt(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, "从红包账户转至QT账户", remark);
			return null;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String selfTransferMgHB(String loginname, Double remit,String betMultiples,Integer deposit) {
		...
		try {
			transferService.addTransferforMg(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, "红包账户金额转至MG", remark);
			return null;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String selfTransferDtHB(String loginname, Double remit,String betMultiples,Integer deposit) {
		...
		try {
			transferService.addTransferforDt(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, "红包账户金额转至DT", remark);
			return null;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String selfTransferSlotHB(String loginname, Double remit,String betMultiples,Integer deposit) {
		...
		try {
			//transferService.addTransferforDt(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, "红包账户金额转至老虎机平台", remark);
			return null;
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		public String selfTransferQtEmigrated(String loginname, Double remit) {
			...
			try {
					transferService.addTransferforQt(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, "", remark);
					return null;
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public String selfTransferTtgEmigrated(String loginname, Double remit) {
		...
		try {
				transferService.addTransferforTt(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, "", remark);
				return null;
		} catch (Exception e) {
			e.printStackTrace();
		}
...
//		else if(!user.getWarnflag().equals(3)){//对于非安全玩家，需要查询看 同姓名 同IP下  三个月内是否领取过体验金
		//查询同IP或者同姓名的所有玩家
		DetachedCriteria dc1=DetachedCriteria.forClass(Users.class);
		dc1.add(Restrictions.or(Restrictions.eq("accountName",user.getAccountName()), Restrictions.eq("registerIp",user.getRegisterIp())));
...
//			Double changeMoney = Math.abs(remit * 0.68) > 888 ? 888 : Math.abs(remit * 0.68);
			Double changeMoney = Math.abs(remit * config.getPercent()) > config.getLimitMoney() ? config.getLimitMoney() : Math.abs(Arith.mul(remit, config.getPercent()));
	...
	public AutoYouHuiVo selfYouHui92(String loginname, Double remit) {
		...
		try {
			DepositPendingResponseBean depositPendingResponseBean = RemoteCaller.depositPendingRequest(loginname, changeMoney+remit, transID, user.getAgcode());
			if(null != depositPendingResponseBean && depositPendingResponseBean.getStatus().equals(ErrorCode.SUCCESS.getCode())){
				RemoteCaller.depositConfirmationResponse(depositPendingResponseBean.getId(), ErrorCode.SUCCESS.getCode(), depositPendingResponseBean.getPaymentid(), user.getAgcode());
				
				transferService.addTransfer(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, depositPendingResponseBean.getPaymentid(), null);
				if (StringUtils.isNotBlank(user.getShippingcode())) {
					transferService.updateUserShippingcodeSql(user);
				}
	...
	public AutoYouHuiVo selfYouHui93(String loginname, Double remit) {
		...
		try {
			DspResponseBean dspResponseBean = RemoteCaller.depositPrepareDspRequest(loginname, changeMoney + remit, transID);
			DspResponseBean dspConfirmResponseBean = null;
			if (dspResponseBean != null && dspResponseBean.getInfo().equals("0")) {
				dspConfirmResponseBean = RemoteCaller.depositConfirmDspRequest(loginname, changeMoney + remit, transID, 1);
				if(null != dspConfirmResponseBean && dspConfirmResponseBean.getInfo().equals("0")){
					transferService.addTransferforDsp(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, "", null);
	...
	public AutoYouHuiVo selfYouHui94(String loginname, Double remit) {
		...
		try {
			DspResponseBean dspResponseBean = RemoteCaller.depositPrepareDspAginRequest(loginname, changeMoney+remit, transID);
			DspResponseBean dspConfirmResponseBean = null;
			if (dspResponseBean != null && dspResponseBean.getInfo().equals("0")) {
				dspConfirmResponseBean = RemoteCaller.depositConfirmDspAginRequest(loginname, changeMoney+remit, transID, 1);
				if(null != dspConfirmResponseBean && dspConfirmResponseBean.getInfo().equals("0")){
					transferService.addTransferforAginDsp(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, "", null);
	...
	public AutoYouHuiVo selfYouHui95(String loginname, Double remit) {
		...
		try {
			DspResponseBean dspResponseBean = RemoteCaller.depositBbinRequest(loginname, remit.intValue()+changeMoney.intValue(), user.getCredit(), transID);
			log.info(dspResponseBean.getInfo());
			if(null != dspResponseBean && dspResponseBean.getInfo().equals("11100")){
				transferService.addTransferforBbin(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, "", "转入成功");
	...
	public AutoYouHuiVo selfYouHui96(String loginname, Double remit){
		...
		if(null != transfers && transfers.size()>0 && null != transfers.get(0)){
			...
			// 转账
			String transID = seqService.generateTransferID();
			String msg = transferService.transferInforEbetIn(transID, loginname, remit, ProposalType.SELFEBET96.getText(), remark);
			...
			try {
				//DspResponseBean dspResponseBean = RemoteCaller.depositBbinRequest(loginname, remit.intValue()+changeMoney.intValue(), user.getCredit(), transID);
				//TODO 开始Ebet转账
				String ebetT = RemoteCaller.tradeEbetRequest(loginname, remit.intValue()+changeMoney.intValue(), "IN", transID);
				log.info("Ebet优惠转账错误消息(成功为空):"+ebetT);
				if(null == ebetT && StringUtils.isEmpty(ebetT)){
					transferService.addTransferforEbet(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, "", "转入成功");
	...
	public AutoYouHuiVo selfYouHui97(String loginname, Double remit) {
		...
		// 转账
		String transID = seqService.generateTransferID();
		String msg = transferService.transferInforEbetIn(transID, loginname, remit, ProposalType.SELFEBET97.getText(), remark);
	...
	public String selfYouHui8Yuan(String loginname , String platform) {
		...
		try {
			if(platform.equals("PT")){
				...
				//remoteCredit = Double.valueOf(MGSUtil.getBalance(loginname,user.getPassword())) ;
		...
		transferDao.addTransferINNewPt8Yuan(Long.parseLong(transID), loginname, user.getCredit(), changeMoney, name , target);

		String pno = seqDao.generatePt8Yuan();
		Proposal proposal = new Proposal(pno, "system", DateUtil.now(), ProposalType.SELFPT8.getCode(),
				user.getLevel(), loginname, changeMoney, user.getAgent(), ProposalFlagType.EXCUTED.getCode(),
				Constants.FROM_FRONT, name, null);
		proposal.setGifTamount(changeMoney);
		proposal.setExecuteTime(new Date());
...
//		 if(!user.getWarnflag().equals(3)){//对于非安全玩家，需要查询看 同姓名 同IP下  三个月内是否领取过体验金
		//查询同IP或者同姓名的所有玩家
		DetachedCriteria dc1=DetachedCriteria.forClass(Users.class);
		dc1.add(Restrictions.or(Restrictions.eq("accountName",user.getAccountName()), Restrictions.eq("registerIp",user.getRegisterIp())));
		List<Users> list = transferService.findByCriteria(dc1);
...
//		}
		...
		if(null != transfers && transfers.size()>0 && null != transfers.get(0)){
			...
			String msg = transferService.selfConpon4TTG(transID, loginname, remit, ProposalType.SELFEBET98.getText(), remark);
			...
			try {
				...
				if(null != b && b){
					transferService.addTransferforTt(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, "", "转入成功");
	...
	public AutoYouHuiVo selfYouHui99(String loginname, Double remit) {
		...
		// 转账
		String transID = seqService.generateTransferID();
		String msg = transferService.selfConpon4TTG(transID, loginname, remit, ProposalType.SELFEBET99.getText(), remark);
	...
	public AutoYouHuiVo selfYouHui4GPI702(String loginname, Double remit) {
		...
		if(null != transfers && transfers.size()>0 && null != transfers.get(0)){
			...
			Offer offer = new Offer(pno, UserRole.MONEY_CUSTOMER.getCode(), loginname, remit, changeMoney , remark);
			Proposal proposal = new Proposal(pno, "system", DateUtil.now(), ProposalType.SELFGPI702.getCode(), user.getLevel(), loginname , remit, user.getAgent(), ProposalFlagType.EXCUTED.getCode(), Constants.FROM_FRONT, null, null);
			proposal.setBetMultiples(config.getBetMultiples().toString());
			proposal.setGifTamount(changeMoney);
			proposal.setExecuteTime(new Date());
	...
	public AutoYouHuiVo selfYouHui4GPI703(String loginname, Double remit) {
		...
		Offer offer = new Offer(pno, UserRole.MONEY_CUSTOMER.getCode(), loginname, remit, changeMoney , remark);
		Proposal proposal = new Proposal(pno, "system", DateUtil.now(), ProposalType.SELFGPI703.getCode(), user.getLevel(), loginname , remit, user.getAgent(), ProposalFlagType.EXCUTED.getCode(), Constants.FROM_FRONT, null, null);
		proposal.setBetMultiples(config.getBetMultiples().toString());
		proposal.setGifTamount(changeMoney);
		proposal.setExecuteTime(new Date());
	...
	public AutoYouHuiVo selfYouHui4GPI704(String loginname, Double remit) {
		...
		Offer offer = new Offer(pno, UserRole.MONEY_CUSTOMER.getCode(), loginname, remit, changeMoney , remark);
		Proposal proposal = new Proposal(pno, "system", DateUtil.now(), ProposalType.SELFGPI704.getCode(), user.getLevel(), loginname , remit, user.getAgent(), ProposalFlagType.EXCUTED.getCode(), Constants.FROM_FRONT, null, null);
		proposal.setBetMultiples(config.getBetMultiples().toString());
		proposal.setGifTamount(changeMoney);
		proposal.setExecuteTime(new Date());
	...
	public AutoYouHuiVo selfYouHui4PT705(String loginname, Double remit) {
		...
		Offer offer = new Offer(pno, UserRole.MONEY_CUSTOMER.getCode(), loginname, remit, changeMoney , remark);
		Proposal proposal = new Proposal(pno, "system", DateUtil.now(), ProposalType.SELFPT705.getCode(), user.getLevel(), loginname , remit, user.getAgent(), ProposalFlagType.EXCUTED.getCode(), Constants.FROM_FRONT, null, null);
		proposal.setBetMultiples(config.getBetMultiples().toString());
		proposal.setGifTamount(changeMoney);
		proposal.setExecuteTime(new Date());
	...
	public AutoYouHuiVo selfYouHui4TTG706(String loginname, Double remit) {
		...
		// 转账
		String transID = seqService.generateTransferID();
		String msg = transferService.selfConpon4TTG(transID, loginname, remit, ProposalType.SELFTTG706.getText(), remark);
...
//		}
		...
		if(null != transfers && transfers.size()>0 && null != transfers.get(0)){
			...
			Double changeMoney = Math.abs(Arith.mul(remit, config.getPercent())) > config.getLimitMoney() ? config.getLimitMoney()
					: Math.abs(Arith.mul(remit, config.getPercent()));
			changeMoney=Arith.round(changeMoney, 2);/*精确到小数点后2位*/
			...
			String msg = transferService.selfCouponQT(transID, loginname, remit, ProposalType.SELFQTFIRST.getText(), remark);
			...
			String codeOne = dfh.utils.StringUtil.getRandomString(3);
			String codeTwo = dfh.utils.StringUtil.getRandomString(3);
			String shippingcode = "NTF" + codeOne + sqlCouponId + codeTwo;
			proposal.setShippingCode(shippingcode);*/
			...
			try {
				...
				if(null != b && QtUtil.RESULT_SUCC.equals(b)){
					transferService.addTransferforQt(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, "", "转入成功");
	...
	public AutoYouHuiVo selfYouHuiQTTwice(String loginname, Double remit) {
		...
		// 转账
		String transID = seqService.generateTransferID();
		String msg = transferService.selfCouponQT(transID, loginname, remit, ProposalType.SELFQTTWICE.getText(), remark);
...
//		}
		...
		if(null != transfers && transfers.size()>0 && null != transfers.get(0)){
			...
			String msg = transferService.selfCouponNT(transID, loginname, remit, ProposalType.SELFNTFIRST.getText(), remark);
			...
			transferService.addTransferforNT(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, "", "转入成功");
			vo.setGiftMoney(Arith.add(remit, changeMoney));
	...
	public AutoYouHuiVo selfYouHuiNTTwice(String loginname, Double remit) {
		...
		// 转账
		String transID = seqService.generateTransferID();
		String msg = transferService.selfCouponNT(transID, loginname, remit, ProposalType.SELFNTTWICE.getText(), remark);
	...
	public String getBirthdayMoney(String loginname) {
		...
		//判断优惠使用剩余次数是否够用
		/*if(!youhuiContinueOrStop(config.getTimes(), config.getTimesflag(), loginname, ProposalType.BIRTHDAY)){

			return null;
		}*/
/*		Date currDate = new Date();
		String year = DateUtil.getYear(currDate);
		String month = DateUtil.getMonth(user.getBirthday());
		String day = DateUtil.getDay(user.getBirthday());
	...
	public String selfTransferMgSign(String loginname, Double remit) {
		...
		// 转账
		String transID = seqService.generateTransferID();
		String msg = transferService.selfConpon4MG4Sign(transID, loginname, remit, ProposalType.SIGNDEPOSIT420.getText(), remark);
		...
		try {
			transferService.addTransferforMg(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, "", remark);
			return null;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String selfTransferDtSign(String loginname, Double remit) {
		...
		// 转账
		String transID = seqService.generateTransferID();
		String msg = transferService.selfConpon4DT4Sign(transID, loginname, remit, ProposalType.SIGNDEPOSIT420.getText(), remark);
		...
		try {
			transferService.addTransferforDt(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, "", remark);
			return null;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String selfTransferSlotSign(String loginname, Double remit) {
		...
		// 转账
		String transID = seqService.generateTransferID();
		String msg = transferService.selfConpon4SLOT4Sign(transID, loginname, remit, ProposalType.SIGNDEPOSIT420.getText(), remark);
		...
		try {
			transferService.addTransferforSlot(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, "", remark);
			return null;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String selfTransferTtgSign(String loginname, Double remit) {
		...
		// 转账
		String transID = seqService.generateTransferID();
		String msg = transferService.selfConpon4TTG4Sign(transID, loginname, remit, ProposalType.SIGNDEPOSIT420.getText(), remark);
	...
	public String selfTransferPtSign(String loginname , Double remit ){
		...
		Offer offer = new Offer(pno, UserRole.MONEY_CUSTOMER.getCode(), loginname, 0.0, remit , remark);
		Proposal proposal = new Proposal(pno, "system", DateUtil.now(), ProposalType.SIGNDEPOSIT420.getCode(), user.getLevel(), loginname , remit, user.getAgent(), ProposalFlagType.EXCUTED.getCode(), Constants.FROM_FRONT, null, null);
	...
	public String selfTransferNTSign(String loginname, Double remit) {
		...
		// 转账
		String transID = seqService.generateTransferID();
		String msg = transferService.selfConponNT4Sign(transID, loginname, remit, ProposalType.SIGNDEPOSIT420.getText(), remark);
	...
	public String selfTransferQTSign(String loginname, Double remit) {
		...
		// 转账
		String transID = seqService.generateTransferID();
		String msg = transferService.selfConpon4QT4Sign(transID, loginname, remit, ProposalType.SIGNDEPOSIT420.getText(), remark);
	...
	public String selfTransferTtgFriend(String loginname, Double remit) {
		...
		// 转账
		String transID = seqService.generateTransferID();
		String msg = transferService.selfConpon4TTG4Friend(transID, loginname, remit, ProposalType.FRIEND390.getText(), remark);
	...
	public String selfTransferPtFriend(String loginname , Double remit ){
		...
		if(null != offers && offers.size()>0){
			...
			Offer offer = new Offer(pno, UserRole.MONEY_CUSTOMER.getCode(), loginname, 0.0, remit , remark);
			Proposal proposal = new Proposal(pno, "system", DateUtil.now(), ProposalType.FRIEND390.getCode(), user.getLevel(), loginname , remit, user.getAgent(), ProposalFlagType.EXCUTED.getCode(), Constants.FROM_FRONT, null, null);
			proposal.setBetMultiples(mul);//----------------流水倍数
			proposal.setGifTamount(0.0);
			proposal.setExecuteTime(new Date());
			proposal.setRemark(remark);
	...
	public String selfTransferNTFriend(String loginname, Double remit) {
		...
		// 转账
		String transID = seqService.generateTransferID();
		String msg = transferService.selfConponNT4Friend(transID, loginname, remit, ProposalType.FRIEND390.getText(), remark);
		log.info("NT签到奖金 转账信息：" + msg);
		if (null != msg) {
			return msg;
		}
	...
	public String selfTransferQtFriend(String loginname, Double remit) {
		...
		// 转账
		String transID = seqService.generateTransferID();
		String msg = transferService.selfConpon4Qt4Friend(transID, loginname, remit, ProposalType.FRIEND390.getText(), remark);
		log.info("QT好友推荐奖金 转账信息：" + msg);
		if (null != msg) {
			return msg;
		}
	...
	public String selfTransferMgFriend(String loginname, Double remit) {
		...
		// 转账
		String transID = seqService.generateTransferID();
		String msg = transferService.selfConpon4Mg4Friend(transID, loginname, remit, ProposalType.FRIEND390.getText(), remark);
	...
	public String selfTransferDtFriend(String loginname, Double remit) {
		...
		// 转账
		String transID = seqService.generateTransferID();
		String msg = transferService.selfConpon4Dt4Friend(transID, loginname, remit, ProposalType.FRIEND390.getText(), remark);
...
//		}
		...
		if(null != transfers && transfers.size()>0 && null != transfers.get(0)){
			...
			String msg = transferService.selfCouponMG(transID, loginname, remit, ProposalType.SELFMGFIRST.getText(), remark);
			...
			transferService.addTransferforMg(Long.parseLong(transID), loginname, user.getCredit(), remit, Constants.IN, Constants.FAIL, "", "转入成功");
			vo.setGiftMoney(Arith.add(remit, changeMoney));
	...
	public AutoYouHuiVo selfYouHuiMGTwice(String loginname, Double remit) {
		...
		// 转账
		String transID = seqService.generateTransferID();
		String msg = transferService.selfCouponMG(transID, loginname, remit, ProposalType.SELFMGTWICE.getText(), remark);
...
//		}
		...
		String msg = transferService.transferDTAndSelfYouHuiInModify(pno, ProposalType.SELFDTFIRST.getText(), transID, loginname, remit, remark);
		...
		Proposal proposal = new Proposal(pno, "system", DateUtil.now(), ProposalType.SELFDTFIRST.getCode(), user.getLevel(), loginname, remit, user.getAgent(), ProposalFlagType.EXCUTED.getCode(), Constants.FROM_FRONT, null, null);
	...
	public AutoYouHuiVo selfYouHuiDTTwice(String loginname, Double remit) {
		...
		String msg = transferService.transferDTAndSelfYouHuiInModify(pno, ProposalType.SELFDTTWICE.getText(), transID, loginname, remit, remark);
		...
		Proposal proposal = new Proposal(pno, "system", DateUtil.now(), ProposalType.SELFDTTWICE.getCode(), user.getLevel(), loginname, remit, user.getAgent(), ProposalFlagType.EXCUTED.getCode(), Constants.FROM_FRONT, null, null);
		proposal.setBetMultiples(String.valueOf(config.getBetMultiples()));
		proposal.setGifTamount(changeMoney);
		proposal.setExecuteTime(new Date());
	...
	public AutoYouHuiVo selfYouHuiDTSpec(String loginname, Double remit) {
		...
		String msg = transferService.transferDTAndSelfYouHuiInModify(pno, ProposalType.SELFDTSPEC.getText(), transID, loginname, remit, remark);
		...
		Proposal proposal = new Proposal(pno, "system", DateUtil.now(), ProposalType.SELFDTSPEC.getCode(), user.getLevel(), loginname, remit, user.getAgent(), ProposalFlagType.EXCUTED.getCode(), Constants.FROM_FRONT, null, null);
		proposal.setBetMultiples(String.valueOf(config.getBetMultiples()));
		proposal.setGifTamount(changeMoney);
		proposal.setExecuteTime(new Date());
	...
	//App下载彩金 
		...
		public String platformAmountAndRecordAPPRewardLog(String loginname , String platform) {
			...
			//验证优惠是否开启
			Double changeMoney = null;
			YouHuiConfig config = queryYouHuiConfigSingle(ProposalType.SELFHELP_APP_PREFERENTIAL.getText() , user.getLevel());
			if(null == config){
				return "优惠配置未开启";
			}
			...
			//[纪录proposal]
			String pno = seqDao.generatePt8Yuan();
			Proposal proposal = new Proposal(pno, "system", DateUtil.now(), ProposalType.SELFHELP_APP_PREFERENTIAL.getCode(),
					user.getLevel(), loginname, changeMoney, user.getAgent(), ProposalFlagType.EXCUTED.getCode(),
					Constants.FROM_FRONT, name, null);
			proposal.setGifTamount(changeMoney);
			proposal.setExecuteTime(new Date());		
		...
		public String selfTransferMgRedRain(String loginname, Double remit) {
			...
			List<RaincouponRecord> list= proposalService.getHibernateTemplate().findByCriteria(dc.addOrder(Order.desc("createtime")));
			String multiples="";
			...
			// 转账
			String transID = seqService.generateTransferID();
			String msg = transferService.selfConpon4MG4RedRain(transID, loginname, remit, ProposalType.SELF_101.getText(), remark);
			if (null != msg) {
				return msg;
			}
			Offer offer = new Offer(pno, UserRole.MONEY_CUSTOMER.getCode(), loginname, 0.0, remit, remark);
		...
		/*	if (remoteCredit >= 5) {
			...
			// 转账
			String transID = seqService.generateTransferID();
			String msg = transferService.selfConpon4DT4RedRain(transID, loginname, remit, ProposalType.SELF_101.getText(), remark);
			if (null != msg) {
				return msg;
			}
			Offer offer = new Offer(pno, UserRole.MONEY_CUSTOMER.getCode(), loginname, 0.0, remit, remark);
			Proposal proposal = new Proposal(pno, "system", DateUtil.now(), ProposalType.SELF_101.getCode(),
					user.getLevel(), loginname, remit, null, ProposalFlagType.EXCUTED.getCode(),
					Constants.FROM_FRONT, null, null);
		 ...
		 */
		@Override
		public String selfTransferCq9RedRain(String loginname, Double remit) {
			Users user = (Users) cs.get(Users.class, loginname);
			AutoYouHuiVo vo = new AutoYouHuiVo();
			if(null != user && (!user.getWarnflag().equals(4))){

			}else{
				return "抱歉，您不能进行CQ9转入。Q" ;
			}
			...
			String password = user.getPassword();
			try {
				remoteCredit = CQ9Util.getBalance(loginname);
				if(null == remoteCredit ){
					return "获取玩家CQ9游戏账户余额错误!";
				}
			} catch (Exception e) {
				e.printStackTrace();
				return "获取玩家CQ9游戏账户余额异常！";
			}
			/*if (remoteCredit >= 5) {
				return "MG平台金额必须小于5元,才能转入！";
			}*/
			...
			String pno = seqDao.generateProposalPno(ProposalType.SELF_101);
			String remark = "红包雨余额转入CQ9平台,"+multiples+"倍流水，转入" + remit;

			// 转账
			String transID = seqService.generateTransferID();
			String msg = transferService.selfConpon4CQ9RedRain(transID, loginname, remit, ProposalType.SELF_101.getText(), remark);
			if (null != msg) {
				return msg;
			}
			Offer offer = new Offer(pno, UserRole.MONEY_CUSTOMER.getCode(), loginname, 0.0, remit, remark);
		 ...
		 */
		@Override
		public String selfTransferPgRedRain(String loginname, Double remit) {
			Users user = (Users) cs.get(Users.class, loginname);
			AutoYouHuiVo vo = new AutoYouHuiVo();
			if(null != user && (!user.getWarnflag().equals(4))){

			}else{
				return "抱歉，您不能进行PG转入。Q" ;
			}
			...
			String password = user.getPassword();
			try {
				remoteCredit = PGUtil.getBalance(loginname);
				if(null == remoteCredit ){
					return "获取玩家PG游戏账户余额错误!";
				}
			} catch (Exception e) {
				e.printStackTrace();
				return "获取玩家PG游戏账户余额异常！";
			}
			/*if (remoteCredit >= 5) {
				return "MG平台金额必须小于5元,才能转入！";
			}*/
			...
			String pno = seqDao.generateProposalPno(ProposalType.SELF_101);
			String remark = "红包雨余额转入PG平台,"+multiples+"倍流水，转入" + remit;

			// 转账
			String transID = seqService.generateTransferID();
			String msg = transferService.selfConpon4PGRedRain(transID, loginname, remit, ProposalType.SELF_101.getText(), remark);
			if (null != msg) {
				return msg;
			}
			Offer offer = new Offer(pno, UserRole.MONEY_CUSTOMER.getCode(), loginname, 0.0, remit, remark);
