...
import com.gsmc.png.model.QtBetVo;
...
public class QtUtil {
	...
	//returnUrl
	private static final String RETURNURL = "http://www.dayunguoji.com";
	private static final String API_USER = "api_dayun";
	private static final String API_PASSWORD = "1YfE9IbR";
	...
	public static String getBalance(String playerId) {
		...
		if(!RESULT_FAIL.equals(backStr)){
			...
			backStr = getPlayerBalance(token, playerId);
			if(!RESULT_FAIL.equals(backStr)){
	...
	public static String getDepositPlayerMoney(String playerId, Double amount, String referenceId){
		...
		if(!RESULT_FAIL.equals(backStr)){
			String token = backStr;
			backStr = prepareTransfer(token, TYPE_CREDIT, referenceId, playerId, amount, CURRENCY);
			if(!RESULT_FAIL.equals(backStr)){
	...
	public static String getWithdrawPlayerMoney(String playerId, Double amount, String referenceId){
		...
		if(!RESULT_FAIL.equals(backStr)){
			String token = backStr;
			backStr = prepareTransfer(token, TYPE_DEBIT, referenceId, playerId, amount, CURRENCY);
			if(!RESULT_FAIL.equals(backStr)){
				JSONObject jsonObj = JSONObject.fromObject(backStr);
				String transferId = jsonObj.getString("id");
				backStr = completeTransfer(token, transferId);
	 ...
	 * @return
	 */
	public static String getGameUrl(String gameId, String playerId, String mode, String clientType){
		String backStr = getAccessToken(API_USER, API_PASSWORD);
		if(!RESULT_FAIL.equals(backStr)){
			String token = backStr;
			backStr = gameLauncher(token, gameId, playerId, CURRENCY, COUNTRY, LANG, mode, clientType, RETURNURL);
			if(!RESULT_FAIL.equals(backStr)){
				revokeAccessToken(token);
				return backStr;
			}
	...
	public static String getBetTotal(String playerId, String fromDateTime, String toDateTime, Integer size, Integer page, Map<String, QtBetVo> map){
		try {
			...
			if(!RESULT_FAIL.equals(backStr)){
				String token = backStr;
				backStr = gameTransactions(token, playerId, fromDateTime, toDateTime, size, page);
				if(!RESULT_FAIL.equals(backStr)){
					JSONObject jsonObj = JSONObject.fromObject(backStr);
	 ...
	 * @return
	 */
	public static String getBetTotalByNGR(String fromDateTime, String toDateTime){
		Map<String, QtBetVo> map = new HashMap<String, QtBetVo>();
		try {
			String backStr = getAccessToken(API_USER, API_PASSWORD);
			if(!RESULT_FAIL.equals(backStr)){
				String token = backStr;
				backStr = playerNGR(token, fromDateTime, toDateTime);
				if(!RESULT_FAIL.equals(backStr)){
					JSONObject jsonObj = JSONObject.fromObject(backStr);
	...
	public static String gameLauncher(String token, String gameId, String playerId, String currency, 
		...
		try {
			...
			map.put("clientType", clientType);map.put("returnUrl", returnUrl);
			JSONObject jsonObject = JSONObject.fromObject(map);
			method.setRequestBody(jsonObject.toString());
			int statusCode = httpClient.executeMethod(method);
			log.info("Response Code: " + statusCode);
	...
	public static void main(String[] args) {
		...
		String backStr = QtUtil.getBetTotalByNGR("2015-12-02T00:00:00", "2015-12-03T00:00:00");
