...
import com.gsmc.png.model.shaba.SBADataTW;
...
public class SBADataProcessorServiceImpl extends UniversalServiceImpl implements ISBADataProcessorService {
	...
	private void save(String last_version_key) {
		...
		if (list1 != null && !list1.isEmpty()) {
			String updatesql = "update api_call_record set 	 = frequency + 1,updatetime = to_char(sysdate,'yyyy-MM-dd HH24:mi:ss') where version_key =:version_key";
	...
	public List getSbaXimaData(String startTime, String endTime, String productdb) {
		String querysql = "select vendor_member_id , sum(stake),sum(winlost_amount)  from " + productdb
				+ " where winlost_datetime >=:startTime and winlost_datetime <=:endTime group by vendor_member_id";
		Map<String, Object> param = new HashMap<String, Object>();
	...
	public void execute() {
		try {
			String version_key = getVersionKey();
			Map map = null;
			if (StringUtils.isNotEmpty(version_key) &&  version_key != null) {
