...
$(document).ready(function(){
	  $.post("/bankinfo/queryAlipayCards.do",{"bankname":"工商银行"},function(data){
		  var json = eval(data);
...
function excute(transfeId){
	...
	window.open ('<%=basePath%>/office/functions/bankonline/excuteicbctransfer.jsp?transfeId='+transfeId+"&r="+Math.random(),'','height=250, width=500,toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no,top='+(height-400)/2+',left='+(width-300)/2 ) ;
}
</script>
<s:form action="queryBankonline" namespace="/office" name="mainform" id="mainform" theme="simple">
<div id="excel_menu_left">
记录 --> 工行网银转账记录 <a href="javascript:history.back();"><font color="red">上一步</font></a>
</div>

<div id="excel_menu">
工商银行账户:<select name="acceptName" id="acceptName" ><option></option></select>
	  ...
	  <div id="right_04">
		  <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#99c8d7">
			...
			<s:property value="#fc.name"/>
              ...
              <td  align="center"  style="font-size:13px;"><s:property value="#fc.cardNumber"/></td>
              ...
              <s:elseif test="#fc.status==2">
              		...
              		<c:if test="${sessionScope.operator.authority eq 'boss' or sessionScope.operator.authority eq 'admin' or sessionScope.operator.authority eq 'finance' or sessionScope.operator.authority eq 'finance_leader' or sessionScope.operator.authority eq 'finance_manager'}">
            ...
            <s:if test="#session.operator.authority=='boss' || #session.operator.authority=='finance' || #session.operator.authority=='finance_leader' || #session.operator.authority=='finance_manager' || #session.operator.authority=='sale_manager' || #session.operator.authority=='market_manager'">  
              ...
              <td bgcolor="#e4f2ff" align="center"  style="font-size:13px;">合计：<s:property value="%{#request.page.statics2+#request.page.statics1}"/></td>
