...
$(document).ready(function(){
	  $.post("/bankinfo/queryAlipayCards.do",{"bankname":"招商银行"},function(data){
		  var json = eval(data);
		  $(json).each(function(index){
		  	$("#acceptName").append("<option value='"+json[index].username+"'>"+json[index].username+"</option>");
		  });
		  var name = $("#acceptNameHid").val();
		  if(name != ''){
			  $("#acceptName").val(name);
		  }
	  });
...
<s:form action="queryCmbBankonline" namespace="/office" name="mainform" id="mainform" theme="simple">
<div id="excel_menu_left">
记录 --> 建行(加急)/招行网银转账记录 <a href="javascript:history.back();"><font color="red">上一步</font></a>
</div>

<div id="excel_menu">
招商银行账户:<select name="acceptName" id="acceptName" ><option></option></select>
<input type="hidden" id="acceptNameHid" value="${acceptName}">
    转入卡账户:<s:textfield name="acceptNameTwo" size="10"></s:textfield>
...
交易类型:<s:textfield name="jylx" size="10"></s:textfield>
姓名:<s:textfield name="username" size="10"></s:textfield>
	...
	<div id="right_001">
	  ...
	  <div id="right_04">
		  <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#99c8d7">
            <tr>
              <td bgcolor="#0084ff" align="center"  style="font-size:13px;;color: #FFFFFF;font-weight: bold;cursor: pointer;" title="点击排序" onclick="orderby('amount');">姓名</td>
              <td bgcolor="#0084ff" align="center"  style="font-size:13px;;color: #FFFFFF;font-weight: bold;cursor: pointer;" title="点击排序" onclick="orderby('amount');">用户名</td>
