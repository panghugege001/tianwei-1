...
<%-- 							<a style="color: red"href='<s:url value="/office/functions/officeaddguestbookNew.jsp"></s:url>'>发帖</a> --%>
						</td>
					</tr>
					<tr align="left">
						...
						<td>
							<s:select cssStyle="width:80px" list="#{'':'','0':'未读','1':'已读'}"
								name="adminUserStatus" listKey="key" listValue="value"
							...
							<div id="right_04">
								<table width="98%" border="0" align="center" cellpadding="0"
									...
									<s:iterator var="guestbook"
										...
										<tr>
											<td align="center">
												<input type="checkbox" name="item" value="<s:property value="#guestbook.id"/>" />
            								</td> 
											...
											<td bgcolor="#e4f2ff" align="center" style="font-size: 13px;">
												<c:if test="${guestbook.isadmin eq 1}">
													<s:property value="#guestbook.username" />
												</c:if>
												<c:if test="${guestbook.isadmin eq 0}">
													客服管理员
												</c:if>
											</td>
											<td bgcolor="#e4f2ff" align="center" style="font-size: 13px;">
											...
											<td bgcolor="#e4f2ff" align="center" style="font-size: 13px;">
												<c:if test="${guestbook.isadmin eq 1}">
													<c:if test="${guestbook.adminstatus eq 0}">(<font
															color="red">客服管理员</font>)未读</c:if>
													<c:if test="${guestbook.adminstatus eq 1}">(<font
															color="red">客服管理员</font>)已读</c:if>
												</c:if>
												<c:if test="${guestbook.isadmin eq 0}">
													<c:if test="${guestbook.userstatus eq 0}">(<s:property
															value="#guestbook.username" />)未读</c:if>
													<c:if test="${guestbook.userstatus eq 1}">(<s:property
															value="#guestbook.username" />)已读</c:if>
												</c:if>
											</td>
											<td bgcolor="#e4f2ff" align="center" style="font-size: 13px;">
												<c:if test="${guestbook.isadmin eq 1}">
											...
											<td bgcolor="#e4f2ff" align="center" style="font-size: 13px;">
												<c:if
													test="${guestbook.adminstatus eq 1 && guestbook.userstatus eq 0}">
													(<s:property value="#guestbook.username" />)没有新的信息
												</c:if>
												<c:if
													test="${guestbook.adminstatus eq 0 && guestbook.userstatus eq 1}">
													(<font color="red"><s:property
															value="#guestbook.username" /> </font>)有新的信息
												</c:if>
												<c:if
													test="${guestbook.adminstatus eq 1 && guestbook.userstatus eq 1}">
													(<s:property value="#guestbook.username" />)没有新的信息
												</c:if>
												<c:if
													test="${guestbook.adminstatus eq 0 && guestbook.userstatus eq 0}">
													(<font color="red"><s:property
															value="#guestbook.username" /> </font>)有新的信息
												</c:if>
											</td>
