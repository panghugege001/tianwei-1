...
<script type="text/javascript">
		...
		function deleteWorks(id) {
			if (window.confirm("确定吗?")) {
				...
				frm.action = "<c:url value='/office/deleteYouHuiTopicInfo.do' />";
				frm.id.value = id;
				frm.submit();
			}
		}
		...
		function batchDelete(btn){
			...
			if(len>0){
				if(confirm("共选中"+len+"条数据，确认删除？删除后将不能恢复！")){
		    		...
		    		$.ajax({ 
				          type: "post", 
				          url: "/office/batchDeleteTopicInfoYouHui.do", 
				          cache: false,  
				          async: false,
				          data:{"ids":ids},
	...
	<body>
		...
		<s:form action="queryTopicStatusList" namespace="/office" name="mainform"
									...
									<s:iterator var="topicInfo"
										...
										<tr>
											...
											<td bgcolor="#e4f2ff" align="center" style="font-size: 13px;">
											 ...
											 <td bgcolor="#e4f2ff" align="center" style="font-size: 13px;">
													<s:property value="#topicInfo[8]" />
											</td>
											<td bgcolor="#e4f2ff" align="center" style="font-size: 13px;">
												<c:if test="${topicInfo[4] eq 0}">(<font
														color="red"><s:property value="#topicInfo[2]" /></font>)未读</c:if>
												<c:if test="${topicInfo[4] eq 1}">(<font
														color="red"><s:property value="#topicInfo[2]" /></font>)已读</c:if>
											</td>
											<td bgcolor="#e4f2ff" align="center" style="font-size: 13px;">
												<s:property value="#topicInfo[3]" />
											</td>
											<td bgcolor="#e4f2ff" align="center" style="font-size: 13px;">
												<s:date name="#topicInfo[5]"
													format="yyyy-MM-dd HH:mm:ss" />
											</td>
											<td bgcolor="#e4f2ff" align="center" style="font-size: 13px;">
												 <a href="javascript:deleteWorks(${topicInfo[0]});">删除</a> 
											</td>
										</tr>
									</s:iterator>
									<tr>
