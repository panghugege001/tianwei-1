...
function JAlert_Progressbar_Dialog(divId) {
	...
	function df_executeSendSms(btn){
		...
		if(len>0){
			var ids = result.join(",") ;
			var html="<table><tr><td class='label_search_td_play' valign='top'>内容：</td><td><textarea name='remark' class='input' id='remark' style='width: 400px; height: 155px;'></textarea></td></tr><tr><td colspan='2'><input type='button' value='短信发送' onclick='dfduanXinQunFaCommit(\""+ids+"\",this);' style='margin-left: 370px;'></div></td></tr></table>";
			$("#JqAlert").html(html);
	...
	function dfduanXinQunFaCommit(ids,btn){
		        btn.disabled=true;
				...
				$.post("/office/sendDFSmsByUsersList.do",{"ids":ids,"content":remark},function(data){
					 btn.disabled=false;
		    			alert(data);
	...
	function haoMaQunFaCommit(btn) {
		...
		var phone = $("#phone").val();
		$.post("/office/sendDFSmsByPhoneList.do", { "phone": phone, "content": remark }, function(data) {
			btn.disabled = false;
			alert(data);
			$("#JqAlert").dialog("close");
			$("#JqAlert").hide();
...
<s:form action="queryUserSmsDF" namespace="/office" name="mainform" id="mainform" theme="simple">
<div id="excel_menu_left">
账户 --> 短信发送用户列表<a href="javascript:history.back();"> <font color="red">上一步</font></a>
...
<input type="button" value="短信发送" onclick="df_executeSendSms(this);"/>
<input type="button" value="手机号码发送" onclick="df_executeSendPhone(this)" />
<div id="excel_menu">
