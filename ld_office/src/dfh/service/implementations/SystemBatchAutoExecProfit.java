...
public class SystemBatchAutoExecProfit extends AbstractGameinfoServiceImpl {

	private Logger log=Logger.getLogger(SystemBatchAutoExecProfit.class);
	...
	public void excuteAutoXimaProposal() throws Exception {
		...
		List<Proposal> notExecProposal = this.getProposalService().getNotExecProfitProposal(starttime, new Date()); 
		...
		if (notExecProposal==null||notExecProposal.size()<=0) {
			...
			for (Proposal proposal : notExecProposal) {
				...
				this.getTradeDao().changeCredit(xima.getLoginname(), xima.getTryCredit(), CreditChangeType.PROFIT.getCode(), proposal.getPno(), "ok");
