...
public class NetpayAction extends SubActionSupport {
	...
	public String submitOrder() {
		...
		if (netpay == null) {
			...
			// String callBackUrl = getRequest().getScheme() + "://" + getRequest().getServerName() + ":" + getRequest().getServerPort() + getRequest().getContextPath() + "/jsp/"
			...
			// 添加订单
			String msg = netpayService.addNetpayOrder(billno, StringUtil.trim(loginname), StringUtil.trim(password), netpayName, Constants.FLAG_TRUE.toString().equals(newAccount), StringUtil
					.trim(partner), amount, StringUtil.trim(aliasName), StringUtil.trim(phone), StringUtil.trim(email), netpayInterfaces.getHamc(netpay.getPuturl(), billno, netpay.getMerno(), amount,
					callBackUrl, netpay.getKey()), getIp());
			if (msg != null) {
				setErrormsg(msg);
			} else {
				log.info("callBackUrl:" + callBackUrl);
				
				// 获取请求URL，并配置请求参数。官方URL
				String redirectURL = netpayInterfaces.requestURL(netpay.getPuturl(), billno, netpay.getMerno(), amount, callBackUrl, netpay.getKey(), getIp());
	...
	public String submitOrderForOpay() {
		...
		if (netpay == null) {
			...
			String callBackUrl = getRequest().getScheme() + "://" + getRequest().getServerName() + ":" + getRequest().getServerPort() + getRequest().getContextPath() + "/asp/callBackOpay.aspx";
			String returnUrl = getRequest().getScheme() + "://" + getRequest().getServerName() + ":" + getRequest().getServerPort() + getRequest().getContextPath() + "/deposit.asp";
			log.info("callBackUrl:" + callBackUrl);
			String msg = netpayService.addNetpayOrder(billno, customer.getLoginname(), null, netpay.getName(), false, null, amount, customer.getAccountName(), customer.getPhone(),
					customer.getEmail(), Opay.getSign(pd_FrpId, netpay.getPuturl(), billno, netpay.getMerno(), amount, callBackUrl, returnUrl, netpay.getCode(), netpay.getKey()), getIp());
			if (msg != null) {
				setErrormsg(msg);
			} else {
				log.info("callBackUrl:" + callBackUrl);
				String requestForm = Opay.requestForm(pd_FrpId, netpay.getPuturl(), billno, netpay.getMerno(), amount, callBackUrl, returnUrl, netpay.getCode(), netpay.getKey());
	...
	public String callBackOpay() {
		...
		String output = "";
		try {
			if (StringUtil.isEmpty(p02_out_ordercode) || StringUtil.isEmpty(p03_payamount) || StringUtil.isEmpty(p10_note) || StringUtil.isEmpty(p11_status)) {
				output = "回调参数不全";
			} else {
				Netpay netpay = (Netpay) netpayService.get(Netpay.class, NetpayTypeEnum.OPAY.getCode());
				if (!Opay.callBackValidate(request, netpay.getCode(), netpay.getKey())) {
					...
					if (p11_status.equals("success")) {
						String msg = netpayService.excuteNetpayOrder(p02_out_ordercode, Double.parseDouble(p03_payamount), true, "壹支付平台的订单流水号:" + p12_ordercode, sign, getIp());
						if (msg == null) {
		...
		try {
			...
			if (r1_Code.equals("1")) {
				...
				if (r9_BType.equals("1")) {
					msg = null;
					msg = netpayService.excuteNetpayOrder(r6_Order, Double.parseDouble(r3_Amt), r1_Code.equals("1"), "易宝单号:" + r2_TrxId, hmac, getIp());
	...
	public String callBackcncard() {
		...
		String c_succmark = request.getParameter("c_succmark");// 交易成功标识 Y-成功 N-失败
		String c_signstr = request.getParameter("c_signstr");// 验证信息
		try {
			if (StringUtil.isEmpty(c_mid) || StringUtil.isEmpty(c_order) || StringUtil.isEmpty(c_orderamount) || StringUtil.isEmpty(c_ymd) || StringUtil.isEmpty(c_moneytype)
					|| StringUtil.isEmpty(c_transnum) || StringUtil.isEmpty(c_succmark) || StringUtil.isEmpty(c_signstr)) {
			...
			if (c_succmark.equalsIgnoreCase("Y")) {
				String msg;
				msg = netpayService.excuteNetpayOrder(c_order, Double.parseDouble(c_orderamount), c_succmark.equalsIgnoreCase("Y"), "云网单号:" + c_transnum, c_signstr, getIp());
				if (msg == null) {
					repsonse.getWriter().println("success");
					return "successUrl";
				} else {
	...
	public String callBacknps() {
		...
		try {
			...
			if (Status.equalsIgnoreCase("2")) {
				String msg;
				msg = netpayService.excuteNetpayOrder(m_orderid, Double.parseDouble(m_oamount), Status.equalsIgnoreCase("2"), "NPS", signMsg, getIp());
				if (msg == null) {
					repsonse.getWriter().println("success");
					return "successUrl";
				} else {
	...
	public String callBackips() {
		...
		try {
			if (StringUtil.isEmpty(billno) || StringUtil.isEmpty(Currency_type) || StringUtil.isEmpty(amount) || StringUtil.isEmpty(date) || StringUtil.isEmpty(succ) || StringUtil.isEmpty(signature)) {
				repsonse.getWriter().println("回调参数不全");
				return "failUrl";
			}
			...
			if (succ.equalsIgnoreCase("Y")) {
				String msg;
				msg = netpayService.excuteNetpayOrder(billno, Double.parseDouble(amount), succ.equalsIgnoreCase("Y"), "IPS单号:" + ipsbillno, signature, getIp());
				if (msg == null) {
					repsonse.getWriter().println("success");
					return "successUrl";
				} else {
	...
	public String callBacktenpay() {
		...
		try {
			if (StringUtil.isEmpty(cmdno) || StringUtil.isEmpty(pay_result) || StringUtil.isEmpty(date) || StringUtil.isEmpty(transaction_id) || StringUtil.isEmpty(sp_billno)
					|| StringUtil.isEmpty(total_fee) || StringUtil.isEmpty(fee_type) || StringUtil.isEmpty(attach) || StringUtil.isEmpty(bargainor_id) || StringUtil.isEmpty(sign)) {
				repsonse.getWriter().println("回调参数不全");
				return "failUrl";
			}
			...
			if (pay_result.equalsIgnoreCase("0")) {
				String msg;
				msg = netpayService.excuteNetpayOrder(billno, Double.parseDouble(total_fee) / 100, pay_result.equalsIgnoreCase("0"), "TENPAY单号:" + transaction_id, sign, getIp());
				if (msg == null) {
					repsonse.getWriter().println("success");
					return "successUrl";
				} else {
