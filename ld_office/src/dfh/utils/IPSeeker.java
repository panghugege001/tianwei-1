 ...
 * 
 * 
 * 用来读取QQwry.dat文件，以根据ip获得好友位置，QQwry.dat的格式是 一. 文件头，共8字节 1. 第一个起始IP的绝对偏移， 4字节 2. 最后一个起始IP的绝对偏移， 4字节 二. "结束地址/国家/区域"记录区 四字节ip地址后跟的每一条记录分成两个部分 1. 国家记录 2. 地区记录 但是地区记录是不一定有的。而且国家记录和地区记录都有两种形式 1. 以0结束的字符串
 * 2. 4个字节，一个字节可能为0x1或0x2 a. 为0x1时，表示在绝对偏移后还跟着一个区域的记录，注意是绝对偏移之后，而不是这四个字节之后 b. 为0x2时，表示在绝对偏移后没有区域记录 不管为0x1还是0x2，后三个字节都是实际国家名的文件内绝对偏移 如果是地区记录，0x1和0x2的含义不明，但是如果出现这两个字节，也肯定是跟着3个字节偏移，如果不是 则为0结尾字符串 三.
...
public class IPSeeker {
	...
	public IPEntry get(int index) {
		long offset = ipBegin + index * IP_RECORD_LENGTH;
		readIP(offset, b4);
		String beginIP = getIpStringFromBytes(b4);
		offset = readLong3();
		readIP(offset, b4);
		String endIP = getIpStringFromBytes(b4);
		IPLocation ipLocation = getIPLocation(offset);
		IPEntry entry = new IPEntry();
		entry.area = ipLocation.area;
		entry.beginIp = beginIP;
		entry.endIp = endIP;
		entry.country = ipLocation.country;
		entry.index = index;
		return entry;
	}
	...
	public  String getRandomIp() {
		...
		IPEntry ipentry = get(index);
		return ipentry.beginIp;
	}
