...
import dfh.action.pay.NetpayAction;
...
public class NewAccountTest extends TestCase{
	...
	{
		initSpring();
		NetpayAction action = new NetpayAction();
		...
		action.setAmount(1000.0);
		action.setPartner("batman");
		action.setBillno("121212");
		String accountNo = "11111";
		try
		{
		assertEquals(3, 2+1);
		assertEquals("success" , action.newAccountConfirmOrder());
