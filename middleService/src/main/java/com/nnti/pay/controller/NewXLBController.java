...
@Controller
@RequestMapping("/xxlb")
public class NewXLBController extends BasePayController {
    ...
    public Response zfb_wx(HttpServletRequest req, @RequestParam(value = "requestData", required = false) String requestData) throws Exception {
        ...
        NewXlbPayVo vo = gson.fromJson(mp.getRemain(), NewXlbPayVo.class);
        ...
        if (usetype == Constant.USE_TYPE_WEB) {
            if (mp.getPayWay().equals(2)) {
            	vo.setService("TRADE.H5PAY");// H5 微信
            }
        }
        ...
        if (("TRADE.H5PAY").equals(vo.getService())) {
            ...
            log.info(orderId + ":xxlb zfb_wx请求URL：" + url);
            log.info(orderId + ":xxlb zfb_wx请求参数：" + paramsStr + "&sign=" + vo.getSign());
    ...
    public String zfb_wx_return(NewXlbPayVo vo, HttpServletRequest req) throws Exception {
    	
    	 log.info("新迅联宝微信回调接收参数：" + MyWebUtils.getRequestParameters(req));
        ...
        log.info("xxlb接收参数：" + MyWebUtils.getRequestParameters(req));
        try {
            ...
            // 验证签名
            Assert.isTrue(sign.equalsIgnoreCase(DigestUtils.signByMD5(srcMsg, mp.getSignKey())), "新迅联宝微信验证签名失败");
