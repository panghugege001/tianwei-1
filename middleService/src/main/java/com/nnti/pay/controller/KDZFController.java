...
import com.nnti.pay.controller.vo.KdPayVo;
...
@Controller
@RequestMapping("/kd")
public class KDZFController extends BasePayController {
    ...
    public Response zfb_wx(HttpServletRequest req, @RequestParam(value = "requestData", required = false) String requestData) throws Exception {
        ...
        KdPayVo vo = gson.fromJson(merchantPay.getRemain(), KdPayVo.class);
        if (usetype == Constant.USE_TYPE_WEB) {
            vo.setP_ChannelId(vo.getP_ChannelId_web());
            loginName = "wap_" + loginName;
        }
        vo.setWxZfb(merchantPay.getMerchantCode(), orderId, orderAmount, orderAmount, 1, "", loginName,
        ...
        Map pays;
        if (usetype == Constant.USE_TYPE_WEB) {
            pays = MyUtils.describe(vo, "P_UserId", "P_OrderId", "P_FaceValue", "P_ChannelId", "P_Price", "Attr1", "Attr2",
                    "P_Quantity", "P_Description", "P_Notic", "P_Result_URL", "P_Notify_URL", "P_PostKey", "apiUrl");
        } else {

            pays = MyUtils.describe(vo, "P_UserId", "P_OrderId", "P_FaceValue", "P_ChannelId", "P_Price",
                    "P_Quantity", "P_Description", "P_Notic", "P_Result_URL", "P_Notify_URL", "P_PostKey", "apiUrl");
        }
        ...
        log.info(orderId + ":kd online_pay请求参数：" + pays);
    ...
    public String zfb_wx_return(KdPayVo vo, HttpServletRequest req) {
        log.info("kd 接收参数：" + MyWebUtils.getRequestParameters(req));
        ...
        try {
            Assert.isTrue(validationTrustIp(req, DictionaryType.KD_TRUST_IP_TYPE.getName()), "回掉的IP，不在白名单内！");

            Assert.isTrue(errCode.equals("0"), ErrorCode.SC_30000_111.getType());
            MerchantPay mp = merchantPayService.findByMerNo(userId);
