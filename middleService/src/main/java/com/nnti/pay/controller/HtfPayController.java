...
import com.nnti.pay.controller.vo.HtfPayVo;
...
@Controller
@RequestMapping("/htf")
public class HtfPayController extends BasePayController {
    ...
    public Response zfb_wx(HttpServletRequest req, @RequestParam(value = "requestData", required = false) String requestData) throws Exception {
        ...
        HtfPayVo vo = gson.fromJson(merchantPay.getRemain(), HtfPayVo.class);
        if (usetype == Constant.USE_TYPE_WEB) {
        	if(merchantPay.getPayPlatform().contains("htfwx")){  
        		vo.setP_ChannelId("33");//wx
        	}
        	else if(merchantPay.getPayPlatform().contains("htfqqH5")){
        ...
        log.info(orderId + ":hft online_pay请求参数：" + pays);
    ...
    public String zfb_wx_return(HtfPayVo vo, HttpServletRequest req) {
        log.info("hft 接收参数：" + MyWebUtils.getRequestParameters(req));
    ...
    public String online_pay_return(HtfPayVo vo, HttpServletRequest req) throws Exception {
