...
import com.nnti.pay.controller.vo.YYFPayVo;
...
@Controller
@RequestMapping("/yyf")
public class YYFController extends BasePayController {
    ...
    public Response online_pay(HttpServletRequest req, @RequestParam(value = "requestData", required = false) String requestData) throws Exception {
        ...
        orderId = mp.getPayPlatform()+orderId.substring(orderId.length()-7);
        YYFPayVo vo = gson.fromJson(mp.getRemain(), YYFPayVo.class);
		...
		vo.setOnline_pay(mp.getMerchantCode(), orderId, orderDate,type, mp.getNotifyUrl(), mp.getShopUrl(), orderAmount, loginName + "," + mp.getId(), "abc", "1", "abc","abc.com");

		Map<String, Object> maps = MyUtils.describe(vo, "pay_amount", "pay_applydate", "pay_bankcode", "pay_callbackurl",
				"pay_memberid", "pay_notifyurl", "pay_orderid", "pay_productnum", "pay_productname", "pay_attach","pay_productdesc","pay_producturl");
		
		String _sign = DigestUtils.getSortedSign(maps,"&key=" + mp.getSignKey());
		vo.setPay_md5sign(_sign);
		
		log.info("签名 " +vo.getPay_md5sign());

		Map pays = MyUtils.describe(vo, "pay_md5sign","pay_amount", "pay_applydate", "pay_bankcode", "pay_callbackurl",
				"pay_memberid", "pay_notifyurl", "pay_orderid", "pay_productnum", "pay_productname", "pay_attach","pay_productdesc","pay_producturl");
