...
import com.nnti.pay.controller.vo.SQPayVo;
...
public class SQPayController extends BasePayController {
    ...
    public Response zfb_wx(HttpServletRequest req, @RequestParam(value = "requestData", required = false) String requestData) throws Exception {
        ...
        SQPayVo vo = gson.fromJson(mp.getRemain(), SQPayVo.class);
        
        ReturnVo returnVo = new ReturnVo();
        
        	
    	vo.setWxZfb(orderAmount, orderId, tradeDate, "WARIII", customerIp, "", mp.getShopUrl(), mp.getNotifyUrl(), loginName,mp.getSignKey());
    	
    	vo.setApiUrl(mp.getApiUrl());

	    String paramsStr = MyUtils.obj2UrlParam(vo, true,"notifyUrl", "orderIp", "orderPrice", "orderTime", "outTradeNo", "payKey",
	             "productName", "productType", "remark", "returnUrl");
	    vo.setSign(DigestUtils.signByMD5(paramsStr));
	     
		
	    Map pays = MyUtils.describe(vo, "notifyUrl", "orderIp", "orderPrice", "orderTime", "outTradeNo", "payKey",
	             "productName", "productType", "remark", "returnUrl", "overTime", "orderIp","apiUrl","sign");
    ...
    public String zfb_wx_return(XlbPayVo vo, HttpServletRequest req) throws Exception {
        ...
        try {
            ...
            // 验证签名
            Assert.isTrue(signMsg.equalsIgnoreCase(DigestUtils.signByMD5(srcMsg, mp.getSignKey())), "迅联宝验证前面失败");
