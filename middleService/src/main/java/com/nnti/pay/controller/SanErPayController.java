...
import com.nnti.pay.controller.vo.SanErResponseVo;
...
/**
 * @author wander
 * @Description 32pay 支付
 */
@Controller
@RequestMapping("/saner")
public class SanErPayController extends BasePayController {
    ...
    public Response zfb_wx(HttpServletRequest req, @RequestParam(value = "requestData", required = false) String requestData) throws Exception {
        ...
        SanErPayVo vo = gson.fromJson(merchantPay.getRemain(), SanErPayVo.class);
        if (usetype == Constant.USE_TYPE_WEB) {
        	if(merchantPay.getPayPlatform().contains("32wx")){  
        		vo.setP_ChannelId("33");//wx
        	}
        	else if(merchantPay.getPayPlatform().contains("32qq")){
        ...
        vo.setWxZfb(merchantPay.getMerchantCode(), orderId, orderAmount, orderAmount, 1,loginName + "," + merchantPay.getId(),
        ...
        log.info(orderId + ":sanerPay online_pay请求参数：" + pays);
    ...
    public String zfb_wx_return(SanErPayVo vo, HttpServletRequest req) {
        log.info("sanerPay 接收参数：" + MyWebUtils.getRequestParameters(req));
        ...
        try {
            Assert.isTrue(validationTrustIp(req, DictionaryType.SANERPay_TRUST_IP_TYPE.getName()), "回掉的IP，不在白名单内！");
            ...
            Assert.isTrue(result.equals("SUCCESS"), "查询订单交易状态失败");
