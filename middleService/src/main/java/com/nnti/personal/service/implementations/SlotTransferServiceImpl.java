...
@Service("slotTransferService")
public class SlotTransferServiceImpl extends BaseService implements IPlatformTransferService,IActivitiesService {
	...
	public String transferIn(AccountTransferDTO dto) throws Exception {
		...
		Const st = constService.get("转账SLOT");
		...
		// 获取SLOT账户余额
		Double remoteCredit = SlotUtil.getBalance(product, loginName);

		if (null == remoteCredit) {

			resultMap.put("message", "获取SLOT账户余额失败，请稍后重试！");
			return mapper.writeValueAsString(resultMap);
		}
		...
		paramsMap.put("platform", PlatformCode.Transfer.getCode(Constant.SLOT));
		paramsMap.put("loginName", loginName);
		paramsMap.put("target", PlatformCode.Transfer.getCode(Constant.SLOT));
		paramsMap.put("platformName", Constant.SLOT);
		...
		paramsMap.put("platforms", Arrays.asList(Constant.MG, Constant.DT, Constant.NT, Constant.QT, Constant.PNG, Constant.PTSW));
		...
		if (null != bigBangList && !bigBangList.isEmpty()) {
			if(remoteCredit >= 5)
				for (BigBang bb : bigBangList) {
					...
					Double realBet = getBetAmount(product, loginName, Constant.SLOT, DateUtil.format(bb.getGetTime()), DateUtil.format(new Date()), null);
		...
		transfer.setTarget(PlatformCode.Transfer.getCode(Constant.SLOT));
		...
		creditLog.setType(CreditChangeType.TRANSFER_SLOT_IN);
	...
	public String transferOut(AccountTransferDTO dto) throws Exception {
		...
		paramsMap.put("loginName", loginName);
		paramsMap.put("target", PlatformCode.Transfer.getCode(Constant.SLOT));
		paramsMap.put("source", Constant.SLOT);
		paramsMap.put("platformName", Constant.SLOT);
		...
		paramsMap.put("platforms", Arrays.asList(Constant.MG, Constant.DT, Constant.NT, Constant.QT, Constant.PNG, Constant.PTSW));
		paramsMap.put("conditionStatus", "2");
		message = this.bigBangBetLimit4Slot(paramsMap);
		if (StringUtils.isNotBlank(message)) {
		...
		message = BalanceUtil.transferOutAccount(product, Constant.SLOT, loginName,null, amount, null);
		...
		Transfer transfer = new Transfer();
		transfer.setId(Long.parseLong(transferId));
		transfer.setSource(PlatformCode.Transfer.getCode(Constant.SLOT));
		...
		CreditLog creditLog = new CreditLog();
		creditLog.setLoginName(loginName);
		creditLog.setType(CreditChangeType.TRANSFER_SLOT_OUT);
		creditLog.setCredit(credit);
	...
	public String redEnvelopeCoupon(AccountTransferDTO dto) throws Exception {
		...
		valuesMap.put("creditType", CreditChangeType.TRANSFER_REDCOUPONS_SLOT);
	...
	public String depositCoupon(AccountTransferDTO dto) throws Exception {
		...
		valuesMap.put("creditType", CreditChangeType.TRANSFER_SLOT_COUPON);
	...
	private String commonCoupon(Map<String, String> valuesMap) throws Exception {
		...
		if (remoteCredit >= 5) {
			...
			resultMap.put("message", Constant.SLOT + "平台金额必须小于5元，才能使用优惠劵！");
			return mapper.writeValueAsString(resultMap);
		}

		paramsMap.put("loginName", loginName);
		paramsMap.put("platform", PlatformCode.Platform.getCode(Constant.SLOT));
		...
		if (ProposalType.SELF_419.getCode().toString().equals(couponType)) {
			...
			creditLogRemark = "referenceNo:" + transferId + ";领取SLOT红包优惠券,优惠劵金额(" + giftAmount + "),优惠券代码(" + couponCode + ")";

			proposalRemark = "SLOT红包优惠券";
			...
			creditLogRemark = "referenceNo:" + transferId + ";领取SLOT" + nf.format(couponConfig.getPercent()) + "存送优惠券,转入游戏金额(" + remit + "),存入金额(" + amount + "),优惠劵金额(" + giftAmount + "),优惠券代码(" + couponCode + ")";

			proposalRemark = "SLOT" + nf.format(couponConfig.getPercent()) + "存送优惠券";
		}
		...
		// 新增投注记录
		PreferentialRecord preferentialRecord = new PreferentialRecord(pno, loginName, PlatformCode.Platform.getCode(Constant.SLOT), null, currentDate, 0);
		...
		newSelfRecord.setPlatform(PlatformCode.Platform.getCode(Constant.SLOT));
	...
	public String experienceGold(SelfExperienceDTO dto) throws Exception {
		...
		Integer type = ProposalType.SELF_779.getCode();
		User user = userService.get(loginName);
		...
		if (remoteCredit >= experienceGoldConfig.getMinMoney()) {
			...
			resultMap.put("message", Constant.SLOT + "平台金额必须小于" + experienceGoldConfig.getMinMoney() + "元，才能使用自助体验金！");
			return mapper.writeValueAsString(resultMap);
		}
	...
	public String christmasActivities(AccountTransferDTO dto) throws Exception {
		...
		// 获取Slot账户余额
		Double remoteCredit = Double.valueOf(SlotUtil.getBalance(product, loginName));
		if (null == remoteCredit) {
			resultMap.put("message", "获取Slot账户余额失败，请稍后重试！");
			return mapper.writeValueAsString(resultMap);
		}
		//过滤一重奏限制5元做法
		if(remoteCredit >= 5){
		   resultMap.put("message", Constant.SLOT + "平台金额必须小于5元，申请赠送金额！");
		   return mapper.writeValueAsString(resultMap);
	    }
