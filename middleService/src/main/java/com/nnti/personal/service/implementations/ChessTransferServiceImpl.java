...
import com.nnti.common.utils.ChessUtil;
...
@Service("chessTransferService")
public class ChessTransferServiceImpl extends BaseService implements IPlatformTransferService,IActivitiesService {
	...
	public String transferIn(AccountTransferDTO dto) throws Exception {
		...
		Const st = constService.get("转账CHESS");

		if (null == st || "0".equals(st.getValue())) {

			message = "CHESS转账正在维护中！";
			resultMap.put("message", message);
			return mapper.writeValueAsString(resultMap);
		}

		// 获取CHESS账户余额
		Double remoteCredit = ChessUtil.getBalance(product, loginName);

		if (null == remoteCredit) {

			message = "获取CHESS账户余额失败，请稍后重试！";
			resultMap.put("message", message);
			return mapper.writeValueAsString(resultMap);
		}
		...
		//String transferId = "1041"+DateUtil.getDateID()+"DY8"+loginName;

		// 新增转账记录
		Transfer transfer = new Transfer();
		transfer.setId(Long.parseLong(transferId));
		transfer.setSource(PlatformCode.ProductSign.getCode(product));
		transfer.setTarget(PlatformCode.Transfer.getCode(Constant.CHESS));
		...
		creditLog.setType(CreditChangeType.TRANSFER_CHESS_IN);
	...
	public String transferOut(AccountTransferDTO dto) throws Exception {
		...
		//String transferId = "1041"+DateUtil.getDateID()+"DY8"+loginName;
		message = BalanceUtil.transferOutAccount(product, Constant.CHESS, loginName,user.getPassword(), amount, transferId);
		...
		Transfer transfer = new Transfer();
		transfer.setId(Long.parseLong(transferId));
		transfer.setSource(PlatformCode.Transfer.getCode(Constant.CHESS));
		...
		CreditLog creditLog = new CreditLog();
		creditLog.setLoginName(loginName);
		creditLog.setType(CreditChangeType.TRANSFER_CHESS_OUT);
		creditLog.setCredit(credit);
