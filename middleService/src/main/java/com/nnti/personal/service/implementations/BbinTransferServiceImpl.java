...
import com.nnti.common.utils.BBinUtils;
...
@Service("bbinTransferService")
public class BbinTransferServiceImpl extends BaseService implements IPlatformTransferService,IActivitiesService {
	...
	public String transferIn(AccountTransferDTO dto) throws Exception {
		...
		Const st = constService.get("转账BBIN");
		...
		transfer.setTarget(PlatformCode.Transfer.getCode(Constant.BBIN));
		...
		creditLog.setType(CreditChangeType.TRANSFER_BBIN_IN);
	...
	public String transferOut(AccountTransferDTO dto) throws Exception {
		...
		//String transferId = "1041"+DateUtil.getDateID()+"DY8"+loginName;
		message = BalanceUtil.transferOutAccount(product, Constant.BBIN, loginName,user.getPassword(), amount, transferId);
		...
		Transfer transfer = new Transfer();
		transfer.setId(Long.parseLong(transferId));
		transfer.setSource(PlatformCode.Transfer.getCode(Constant.BBIN));
		...
		CreditLog creditLog = new CreditLog();
		creditLog.setLoginName(loginName);
		creditLog.setType(CreditChangeType.TRANSFER_BBIN_OUT);
		creditLog.setCredit(credit);
