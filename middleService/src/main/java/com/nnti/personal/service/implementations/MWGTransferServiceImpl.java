...
@Service("mwgTransferService")
public class MWGTransferServiceImpl extends BaseService implements IPlatformTransferService {

//	private static Logger log = Logger.getLogger(MWGTransferServiceImpl.class);
	...
	public String transferIn(AccountTransferDTO dto) throws Exception {
		...
		Const st = constService.get("转账MWG");
		...
		// 获取MWG账户余额
		Double remoteCredit = MWGUtils.getUserBalance(product, loginName);

		if (null == remoteCredit) {

			message = "获取MWG账户余额失败，请稍后重试！";
			resultMap.put("message", message);
			return mapper.writeValueAsString(resultMap);
		}
		...
		transfer.setTarget(PlatformCode.Transfer.getCode(Constant.MWG));
		...
		creditLog.setType(CreditChangeType.TRANSFER_MWG_IN);
	...
	public String transferOut(AccountTransferDTO dto) throws Exception {
		...
		message = BalanceUtil.transferOutAccount(product, Constant.MWG, loginName,null, amount, transferId);
		...
		Transfer transfer = new Transfer();
		transfer.setId(Long.parseLong(transferId));
		transfer.setSource(PlatformCode.Transfer.getCode(Constant.MWG));
		...
		CreditLog creditLog = new CreditLog();
		creditLog.setLoginName(loginName);
		creditLog.setType(CreditChangeType.TRANSFER_MWG_OUT);
		creditLog.setCredit(credit);
