...
public class NTDepositServiceImpl extends BaseService implements IPlatformDepositService {
	...
	public String firstDeposit(SelfDepositDTO dto) throws Exception {
		...
		// 首存优惠限制
		String msg = this.firstLimit(loginName, Constant.NT, type, id);
	...
	private String common(SelfDepositDTO dto, String preferentialType, String description) throws Exception {
		...
		if (remoteCredit >= 5) {
			...
			resultMap.put("message", Constant.NT + "平台金额必须小于5元，才能自助优惠！");
			return mapper.writeValueAsString(resultMap);
		}
		...
		params.put("platform", PlatformCode.Platform.getCode(Constant.NT));
		...
		String remark = "自助" + Constant.NT + preferentialType + "," + config.getBetMultiples() + "倍流水,存" + amount + "送" + changeMoney + "(self-help NT " + description + " preferential)";
		...
		log.info("玩家" + loginName + "领取" + Constant.NT + preferentialType + "前的有效总投注额为：" + betAmount);
		...
		resultMap.put("message", "自助" + Constant.NT + preferentialType + "成功！");
