...
@Service("swDepositService")
public class SWDepositServiceImpl extends BaseService implements IPlatformDepositService {
	...
	public String firstDeposit(SelfDepositDTO dto) throws Exception {
		...
		// 首存优惠限制
		String msg = this.firstLimit(loginName, Constant.SW, type, id);
	...
	private String common(SelfDepositDTO dto, String preferentialType, String description) throws Exception {
		...
		if (remoteCredit >= 5) {
			...
			resultMap.put("message", Constant.SW + "平台金额必须小于5元，才能自助优惠！");
			return mapper.writeValueAsString(resultMap);
		}
		...
		params.put("platform", PlatformCode.Platform.getCode(Constant.SW));
		...
		String remark = "自助" + Constant.SW + preferentialType + "," + config.getBetMultiples() + "倍流水,存" + amount + "送" + changeMoney + "(self-help SW " + description + " preferential)";
		...
		resultMap.put("message", "自助" + Constant.SW + preferentialType + "成功！");
