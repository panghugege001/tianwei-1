...
@Service("ebetTransferService")
public class EBETTransferServiceImpl extends BaseService implements IPlatformTransferService {

//	private static Logger log = Logger.getLogger(EBETTransferServiceImpl.class);
	...
	public String transferIn(AccountTransferDTO dto) throws Exception {
		...
		Const st = constService.get("转账EBETAPP");

		if (null == st || "0".equals(st.getValue())) {

			message = "EBETAPP转账正在维护中！";
			resultMap.put("message", message);
			return mapper.writeValueAsString(resultMap);
		}

		// 获取EBETAPP账户余额
		Double remoteCredit = EBETUtil.getBalance(product, loginName);

		if (null == remoteCredit) {

			message = "获取EBETAPP账户余额失败，请稍后重试！";
			resultMap.put("message", message);
			return mapper.writeValueAsString(resultMap);
		}
		...
		transfer.setTarget(PlatformCode.Transfer.getCode(Constant.EBETAPP));
		...
		creditLog.setType(CreditChangeType.TRANSFER_EBETAPP_IN);
	...
	public String transferOut(AccountTransferDTO dto) throws Exception {
		...
		message = BalanceUtil.transferOutAccount(product, Constant.EBETAPP, loginName,null, amount, transferId);
		...
		Transfer transfer = new Transfer();
		transfer.setId(Long.parseLong(transferId));
		transfer.setSource(PlatformCode.Transfer.getCode(Constant.EBETAPP));
		...
		CreditLog creditLog = new CreditLog();
		creditLog.setLoginName(loginName);
		creditLog.setType(CreditChangeType.TRANSFER_EBETAPP_OUT);
		creditLog.setCredit(credit);
