...
@Service("slotDepositService")
public class SlotDepositServiceImpl extends BaseService implements IPlatformDepositService {
	...
	public String firstDeposit(SelfDepositDTO dto) throws Exception {
		...
		// 首存优惠限制
		String msg = this.firstLimit(loginName, Constant.SLOT, type, id);
	...
	private String common(SelfDepositDTO dto, String preferentialType, String description) throws Exception {
		...
		if (remoteCredit >= 5) {
			...
			resultMap.put("message", Constant.SLOT + "平台金额必须小于5元，才能自助优惠！");
			return mapper.writeValueAsString(resultMap);
		}
		...
		params.put("platform", PlatformCode.Platform.getCode(Constant.SLOT));
		...
		String remark = "自助" + Constant.SLOT + preferentialType + "," + config.getBetMultiples() + "倍流水,存" + amount + "送" + changeMoney + "(self-help SLOT " + description + " preferential)";
		...
		String[] productArr = new String[] { "yl" };
		List<String> productList = Arrays.asList(productArr);

		if (productList.contains(product)) {
			resultMap.put("message", "自助SW "+ preferentialType + "成功！");
		}else{
			resultMap.put("message", "自助" + Constant.SLOT + preferentialType + "成功！");
		}
