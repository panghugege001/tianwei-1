...
public class PTDepositServiceImpl extends BaseService implements IPlatformDepositService {
	...
	public String firstDeposit(SelfDepositDTO dto) throws Exception {
		...
		// 首存优惠限制
		String msg = this.firstLimit(loginName, Constant.PT, type, id);
	...
	private String common(SelfDepositDTO dto, String preferentialType, String description) throws Exception {
		...
		if (remoteCredit >= 5) {
			...
			resultMap.put("message", Constant.PT + "平台金额必须小于5元，才能自助优惠！");
			return mapper.writeValueAsString(resultMap);
		}
		...
		params.put("platform", PlatformCode.Platform.getCode(Constant.PT));
		...
		params.put("platform", Constant.PT);
		params.put("conditionStatus", "2");
		params.put("status", "3");

		// 更新大爆炸数据为已处理
		bigBangService.updateList(params);
		...
		String remark = "自助" + Constant.PT + preferentialType + "," + config.getBetMultiples() + "倍流水,存" + amount + "送" + changeMoney + "(self-help PT " + description + " preferential)";
		...
		String codeOne = StringUtil.getRandomString(3);
		String codeTwo = StringUtil.getRandomString(3);
		String shippingCode = sign + codeOne + sqlCouponId + codeTwo;
		...
		log.info("玩家" + loginName + "领取" + Constant.PT + preferentialType + "前的有效总投注额为：" + betAmount);
		// 新增投注记录
		PreferentialRecord preferentialRecord = new PreferentialRecord(pno, loginName, PlatformCode.Platform.getCode(Constant.PT), betAmount, currentDate, 0);
		...
		resultMap.put("message", "自助" + Constant.PT + preferentialType + "成功！");
