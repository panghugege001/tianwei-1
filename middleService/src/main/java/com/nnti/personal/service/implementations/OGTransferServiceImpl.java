...
@Service("ogTransferService")
public class OGTransferServiceImpl extends BaseService implements IPlatformTransferService {
	...
	public String transferIn(AccountTransferDTO dto) throws Exception {
		...
		Const st = constService.get("转账OG");
		...
		// 获取OG账户余额
		Double remoteCredit = OGUtils.getBalance(product, loginName);

		if (null == remoteCredit) {

			message = "获取OG账户余额失败，请稍后重试！";
			resultMap.put("message", message);
			return mapper.writeValueAsString(resultMap);
		}
		...
		transfer.setTarget(PlatformCode.Transfer.getCode(Constant.OG));
		...
		creditLog.setType(CreditChangeType.TRANSFER_OG_IN);
	...
	public String transferOut(AccountTransferDTO dto) throws Exception {
		...
		message = BalanceUtil.transferOutAccount(product, Constant.OG, loginName,null, amount, transferId);
		...
		Transfer transfer = new Transfer();
		transfer.setId(Long.parseLong(transferId));
		transfer.setSource(PlatformCode.Transfer.getCode(Constant.OG));
		...
		CreditLog creditLog = new CreditLog();
		creditLog.setLoginName(loginName);
		creditLog.setType(CreditChangeType.TRANSFER_OG_OUT);
		creditLog.setCredit(credit);
